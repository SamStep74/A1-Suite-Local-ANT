...
- Latest verification from `~/dev/A1-Suite-Local`: connector endpoint guard focused suite `node --test --test-name-pattern "integration connector rejects credentialed endpoint URLs" test/api.test.js` = 1 pass; `npm test` = 354 pass, 0 fail, 0 cancelled; `npm run build:ui` = pass; `ARMOSPHERA_ONE_DB=/tmp/a1-suite-connector-legacy-url-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` = pass; `node --check server/app.js && node --check test/api.test.js && git diff --check` = pass.
...
- Last clean full-suite run from `~/dev/A1-Suite-Local` at `f086d64`: **354 tests / 354 pass / 0 fail / 0 cancelled**.
