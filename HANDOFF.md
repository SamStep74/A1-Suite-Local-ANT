...
_Last updated: 2026-06-02 · main after app-assignment enabled-value guard · 37 tags · **360 tests (360 pass, 0 fail, 0 cancelled)**_
...
### Hardening (production-readiness pass — 37 slices)
...
- Latest app-assignment enabled-value guard verification from `~/dev/A1-Suite-Local`: focused `node --test --test-name-pattern "owner can update app assignment|app assignment rejects non-boolean enabled values|app assignment rejects unknown roles|owner can create an access review" test/api.test.js` = 4 pass; `node --test test/api.test.js` = 172 pass, 0 fail; `npm test` = 360 pass, 0 fail, 0 cancelled; `npm run build:ui` = pass; `ARMOSPHERA_ONE_DB=/tmp/a1-suite-app-assignment-enabled-guard-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` = pass, apps=10; `node --check server/app.js && node --check test/api.test.js && git diff --check` = pass.
...
- Last clean full-suite run from `~/dev/A1-Suite-Local` at app-assignment enabled-value guard checkpoint: **360 tests / 360 pass / 0 fail / 0 cancelled**.
