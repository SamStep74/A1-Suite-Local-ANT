...
describe("summariseItemStock", () => {
  ...
  // Branch: when availableQuantity is non-finite on a row, the reducer
  // must fall through to quantity (status.ts:51).
  it("falls back to quantity when availableQuantity is non-finite", () => {
    expect(
      summariseItemStock([
        { availableQuantity: Number.NaN, quantity: 6 },
        { availableQuantity: 3, quantity: 99 },
      ]),
    ).toEqual({ totalAvailable: 9, health: "low" });
  });

  // Branch: when BOTH availableQuantity and quantity are non-finite
  // on a row, the reducer falls through to 0 (status.ts:51, second
  // operand of the chain).
  it("treats rows with no usable quantity as zero", () => {
    expect(
      summariseItemStock([
        { availableQuantity: Number.NaN, quantity: Number.NaN },
        { availableQuantity: 4 },
      ]),
    ).toEqual({ totalAvailable: 4, health: "low" });
  });
});
...
describe("indexByCatalogItemId", () => {
  ...
  // Branch: when an item's stockBalances is null, the loader falls
  // through to [] (status.ts:74).
  it("treats a null stockBalances as no balances", () => {
    const out = indexByCatalogItemId([
      { id: "n", stockBalances: null },
      { id: "u" }, // undefined
    ]);
    expect(out.n).toEqual({ totalAvailable: 0, health: "unknown" });
    expect(out.u).toEqual({ totalAvailable: 0, health: "unknown" });
  });
});
...
describe("totalStockByItemId", () => {
  ...
  // Branch: when availableQuantity is non-finite on a balance, the
  // aggregation must fall through to quantity (status.ts:89).
  it("falls back to quantity when availableQuantity is non-finite", () => {
    const totals = totalStockByItemId([
      {
        id: "sb-a",
    ...
    expect(totals["ci-x"]).toBe(7);
  });

  // Branch: when BOTH availableQuantity and quantity are non-finite
  // on a balance, the aggregation falls through to 0 (status.ts:89,
  // second operand of the chain).
  it("treats balances with no usable quantity as zero", () => {
    const totals = totalStockByItemId([
      {
        id: "sb-z",
        catalogItemId: "ci-y",
