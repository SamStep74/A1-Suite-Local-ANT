...
- `GET /api/pos/workspace`, `GET /api/pos/cash-sessions`, `POST /api/pos/cash-sessions`, and `POST /api/pos/cash-sessions/:id/close` provide the bounded cash-session spine: open a drawer, list recent sessions, and close with Z-report-like fiscal evidence while explicitly marking sale, refund, stock, receipt-printing, offline replay, and ledger posting as not implemented yet.
...
- The ERP comparison plan now marks first Armenian POS cash-session closeout evidence as shipped while leaving POS sale posting, refunds, offline replay, receipt printing, cash/card split, inventory decrement, and revenue/VAT/cash ledger posting as remaining Phase 2 scope.
