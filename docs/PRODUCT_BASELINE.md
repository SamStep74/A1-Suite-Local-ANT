...
- CRM lead path IDs must be lowercase alphanumeric/hyphen strings; route-reachable unsafe characters are rejected with `400`, and overlong route segments are rejected before conversion work can run.
...
- Rejected malformed CRM lead conversion ID requests keep submitted path/body secrets out of error bodies, preserve no-side-effect behavior for both app-level `400` rejections and router-level overlong-segment `404` rejections, and leave valid conversion metadata, default-title bounds, customer profile creation, and audit behavior unchanged.
