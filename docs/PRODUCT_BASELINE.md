...
- Verification for the checkpoint: syntax checks pass for the RU payroll engine and S7/S5/vendor tests; focused RU payroll/locale/kopeck regression pass (`node --test --test-concurrency=4 --test-timeout=60000 test/localization-ru-vendor-wiring.test.js test/locale.test.js test/kopeck-s7-ru-tax-rounding.test.js test/kopeck-s5-app-validators.test.js test/payroll-endpoints.test.js test/ledger-ru-payroll-contributions.test.js`, 32 pass, 0 fail); `git diff --check` pass; full `npm test` pass (799 pass, 0 fail, 0 cancelled); `npm run build:ui` pass with the existing Vite large-chunk warning; `ARMOSPHERA_ONE_DB=/tmp/a1-suite-kopeck-s7-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` pass (`smoke ok: Armosphera Demo Clinic, apps=12, kpis=4`).
