...
- The suite now has tenant-scoped `catalog_categories` and `catalog_items` tables as the first ERP catalog master-data spine after the Odoo comparison plan.
...
- The suite now has tenant-scoped `stock_locations`, `stock_quants`, and `stock_moves` tables as the first Odoo-parity inventory ledger spine after the catalog master.
...
- Verification for the checkpoint: `node --check scripts/ra-localization.js && node --check test/ra-localization-cli.test.js && node --test test/ra-localization-cli.test.js` pass (6 pass, 0 fail, 0 cancelled); final all-merged verification is covered by Slice 364.
...
- The Finance UI now renders an RA chart-of-accounts panel with class counts, official source metadata, and operating account anchors, and the opening-balance form derives its choices from official asset/liability accounts instead of hardcoded legacy values.
- Regression coverage proves full official chart seeding, `226` VAT posting, legacy `526` read compatibility, chart endpoint auth/shape, VAT-return packet extraction, and opening-balance compatibility.
- Verification for the checkpoint: syntax checks pass for `server/ledger.js`, `server/app.js`, `test/ledger.test.js`, `test/ledger-endpoints.test.js`, and `test/vat-return-api.test.js`; focused official-chart ledger/API suite pass (`node --test --test-concurrency=1 test/ledger.test.js test/ledger-endpoints.test.js test/finance-reports.test.js test/vat-return-api.test.js test/armenia-chart-of-accounts.test.js test/opening-balances.test.js test/opening-balances-endpoints.test.js`, 39 pass, 0 fail, 0 cancelled); later focused all-merged RA finance/localization/payroll suite pass (`node --test test/ra-localization-cli.test.js test/armenia-payroll.test.js test/armenia-chart-of-accounts.test.js test/localization.test.js test/ledger.test.js test/ledger-endpoints.test.js test/vat-return-api.test.js test/finance-reports.test.js`, 52 pass, 0 fail, 0 cancelled); `git diff --check` pass; full `npm test` pass (634 pass, 0 fail, 0 cancelled); `npm run build:ui` pass with the existing Vite large-chunk warning; `ARMOSPHERA_ONE_DB=/tmp/a1-suite-ra-finance-payroll-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` pass (`smoke ok: Armosphera Demo Clinic, apps=10, kpis=4`); Playwright browser proof on `http://127.0.0.1:4192/app/finance` renders `RA chart of accounts`, `623 accounts`, official `226` indirect-tax account text, no legacy `526 · ԱԱՀ դեբետ`, and no unexpected console errors after the expected pre-login `401`.
