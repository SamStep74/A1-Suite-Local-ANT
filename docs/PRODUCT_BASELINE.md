...
- Verification for the checkpoint: `node --check server/app.js` pass; `node --check test/api.test.js` pass; `git diff --check` pass; focused clinic subsequent-ongoing-renewal official invoice posting packet regression (`accountant can record subsequent ongoing renewal official HayHashvapah invoice posting packet from subsequent ongoing draft`) pass (1 target test across 48 files); full `npm test` pass (465 pass, 0 fail, 0 cancelled); `npm run build:ui` pass with the existing Vite large-chunk warning; `ARMOSPHERA_ONE_DB=/tmp/a1-suite-clinic-subsequent-ongoing-renewal-official-invoice-draft-packet-id-guard-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` pass (`smoke ok: Armosphera Demo Clinic, apps=10, kpis=4`).
