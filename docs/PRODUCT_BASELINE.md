...
- Verification for the checkpoint: `node --check server/db.js`, `node --check server/app.js`, and `node --check test/catalog.test.js` pass; focused catalog suite pass (`node --test test/catalog.test.js`, 4 pass, 0 fail, 0 cancelled); broader API/catalog/public quote suite pass (`node --test test/api.test.js test/catalog.test.js test/public-quote-ratelimit.test.js`, 246 pass, 0 fail, 0 cancelled); full `npm test` pass (557 pass, 0 fail, 0 cancelled); `npm run build:ui` pass with the existing Vite large-chunk warning; `ARMOSPHERA_ONE_DB=/tmp/a1-suite-catalog-kernel-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` pass (`smoke ok: Armosphera Demo Clinic, apps=10, kpis=4`).
