...
- Refund posting marks the original sale `refunded_full`, preserves original sale posting evidence, copies original sale-line and source stock-move evidence into refund lines, and explicitly keeps inventory restock, fiscal refund submission, receipt printing, and ledger posting as `not-posted` future work.
- The modern POS workspace now lets the operator record refund evidence for the last posted sale, posts to `/api/pos/sales/:id/refund`, renders refund status, total, method, cash adjustment, inventory/ledger `not-posted` evidence, refreshes the open session, and locks duplicate refund entry after success.
...
- The ERP comparison plan now marks first POS full-sale refund evidence as shipped while keeping voids, partial refunds, return stock flows, terminal/fiscal submission, payment split accounting, offline replay, and revenue/VAT/cash/bank ledger journals deferred.
