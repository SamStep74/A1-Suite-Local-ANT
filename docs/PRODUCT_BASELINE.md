...
- App launcher with CRM, Finance, Desk, Campaigns, Projects, Catalog & Inventory, POS, People, Docs, Analytics, Flow.
- Audit events.
...
### Slice 412 - Project Profitability Field-Visit Cost Evidence
- Service field visits can now carry an optional `projectId` link, validated against the visit customer on create and PATCH, with `null`/empty PATCH clearing the link without breaking existing unlinked dispatch workflows.
- Existing local databases are upgraded safely: `service_field_visits.project_id` is added before the new project index is created, and the compatibility regression covers old-shaped visit tables that predate the project link.
- `GET /api/projects/:id/profitability` now includes `fieldVisitCostTotal`, `fieldVisitCount`, and `fieldVisitCostEvidence`, scoped only to visits explicitly linked to the project rather than inferred from the customer.
- Project profitability uses the request `costRate` to convert linked visit scheduled labor minutes into an estimated field-visit labor cost; travel and material costs stay zero until travel policies and inventory consumption links exist.
- Ledger evidence remains accounting-honest: field-visit labor is marked `not-posted`, while posted accounting-grade allocation across payroll, procurement, inventory consumption, and ledger journals remains a future depth gap.
- The modern Projects billing panel now renders field-visit cost in the cost breakdown and shows linked visit evidence with case, technician, scheduled/labor minutes, cost buckets, and not-posted status.
- Verification for the checkpoint: syntax checks pass for `server/db.js` and `server/app.js`; focused backend migration/project profitability/field-visit regression passes (`node --test --test-concurrency=4 --test-timeout=60000 --test-name-pattern="service field visit tables receive project link|project.*profitability|field visit|project/customer|cost allocation" test/api.test.js test/project-billing.test.js test/service-cases.test.js`, 12 pass, 0 fail); focused modern schema/Projects route regression passes (`npm --prefix web-modern run test -- src/lib/api/schemas.test.ts src/routes/app/projects/-index.test.tsx`, 119 pass, 0 fail); full backend test suite passes (`npm test`, 1501 pass, 0 fail); full modern web test suite passes (`npm --prefix web-modern run test`, 2584 pass, 0 fail); `npm --prefix web-modern run typecheck` passes; `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; `git diff --check` passes; direct egress contract check passes (`node scripts/check-egress-policy-contract.mjs`, `failing_checks=0`); `ARMOSPHERA_ONE_DB=/tmp/a1-suite-project-field-visit-cost-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=15, kpis=4`).

### Slice 413 - Field-Service Material Consumption Evidence
- Stock moves now carry an optional `serviceFieldVisitId` link, persisted as `stock_moves.service_field_visit_id` with startup repair for older local databases before the new service-visit stock-move index is created.
- Inventory posting validates linked field visits inside the tenant and only allows consumption-style move types (`delivery`, `outbound`, `scrap`) to count as service material usage; transfers remain stock movement evidence, not service consumption.
- Field visit read paths now roll posted linked stock moves into `costAllocation.materialCost`, `totalCost`, and bounded `materialEvidence`, including catalog identity, quantity/cost, movement reason/reference, source, timestamp, and the existing stock valuation journal evidence when present.
- Project profitability now rolls linked field-visit material costs into `fieldVisitCostTotal` and `costTotal` alongside the existing scheduled-window labor cost rate, while preserving `not-posted-to-ledger` for the field-service allocation itself.
- The modern API schemas accept material evidence, service-visit-linked stock moves, and the full backend move-type vocabulary; the Inventory moves view now filters and styles `inbound`, `outbound`, and `return` rows in addition to the existing move types.
- The ERP comparison plan now marks linked stock-move material consumption evidence as present while leaving posted accounting-grade service-cost allocation across payroll, travel/procurement, and service ledger journals as the remaining field-service depth gap.
- Verification for the checkpoint: syntax checks pass for `server/db.js` and `server/app.js`; focused backend migration/material/profitability/field-visit regression passes (`node --test --test-concurrency=4 --test-timeout=60000 --test-name-pattern="stock move tables receive service visit link|linked delivery|project.*profitability|field visit|cost allocation" test/api.test.js test/inventory.test.js test/project-billing.test.js test/service-cases.test.js`, 14 pass, 0 fail); focused modern schema/Inventory route regression passes (`npm --prefix web-modern run test -- src/lib/api/schemas.test.ts src/routes/app/inventory/-index.test.tsx`, 94 pass, 0 fail); full backend test suite passes (`npm test`, 1503 pass, 0 fail); full modern web test suite passes (`npm --prefix web-modern run test`, 2584 pass, 0 fail); `npm --prefix web-modern run typecheck` passes; `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; `git diff --check` passes; direct egress contract check passes (`node scripts/check-egress-policy-contract.mjs`, `failing_checks=0`); `ARMOSPHERA_ONE_DB=/tmp/a1-suite-field-service-material-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=15, kpis=4`).

### Slice 414 - Field-Service Material Ledger Status Evidence
- Field-service material cost allocation now derives the materials bucket status from existing linked stock valuation journals instead of treating every material bucket as `not-posted`.
- When every linked service material stock move has matching `stock_move_valuation` evidence that sums to the material cost, the materials ledger mapping reports `posted`, `postedAmount`, `unpostedAmount`, valuation posting IDs, and the `711`/`216` expense/inventory evidence already created by inventory posting.
- Partial or missing stock valuation evidence stays explicit with `partial` or `not-posted` material status, while labor and travel buckets continue to remain `not-posted` until payroll/travel cost policies and service-cost ledger journals are implemented.
- The Projects billing panel and mobile dispatch shell now display the material bucket status (`materials posted`, `materials partial`, or `materials not-posted`) directly, rather than inferring material accounting state from the broader `not-posted-to-ledger` limitation.
- The modern API schema now gives field-service ledger mappings a typed `bucket`/`status` contract while preserving passthrough evidence fields for posted amount, unposted amount, account, and valuation posting metadata.
- The ERP comparison plan now marks linked stock-move material valuation-status evidence as present while leaving posted accounting-grade service-cost allocation across payroll, travel/procurement, and service ledger journals as the remaining field-service depth gap.
- Verification for the checkpoint: syntax check passes for `server/app.js`; focused backend material/profitability/field-visit regression passes (`node --test --test-concurrency=4 --test-timeout=60000 --test-name-pattern="material|cost allocation|project.*profitability|field visit" test/inventory.test.js test/project-billing.test.js test/service-cases.test.js`, 12 pass, 0 fail); focused modern schema/Projects/Dispatch regression passes (`npm --prefix web-modern run test -- src/lib/api/schemas.test.ts src/routes/app/projects/-index.test.tsx src/routes/app/desk/-dispatch.test.tsx`, 137 pass, 0 fail); full backend test suite passes (`npm test`, 1503 pass, 0 fail); full modern web test suite passes (`npm --prefix web-modern run test`, 2584 pass, 0 fail); `npm --prefix web-modern run typecheck` passes; `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; `ARMOSPHERA_ONE_DB=/tmp/a1-suite-field-material-ledger-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=15, kpis=4`).

### Slice 415 - Purchase Billed-Return Credit Note Evidence
- Billed purchase-order returns now stay on the existing `/api/purchase/orders/:id/return` path but require a finance operator, keep the PO status `billed`, and require an explicit return reference for idempotent evidence.
- The return transaction now atomically creates the supplier return stock move, `purchase_returns` row, linked `purchase_credit_notes` row (`po_id`, `bill_id`, `return_id`, amount, posted date), and AP reversal ledger journals.
- Credit-note posting debits payable (`521`) and credits the original purchase expense/input VAT accounts (`711`/`226`) in the returned-line proportions, while posted credit notes reduce finance payables outstanding without being counted as cash payments.
- Purchase order and analytics payloads now expose return credit-note evidence, count, amount, linked return/bill IDs, status, posted date, and ledger posting IDs; admin backups now include `purchase_credit_notes`.
- The modern Purchase detail page now renders a read-only billed-return credit-note panel, and Purchase analytics shows return credit-note amount/count alongside returned quantity.
- Regression coverage proves finance-role gating for billed returns, operator-safe pre-bill returns, duplicate-reference idempotency, conflicting-reference rejection, closed-period rollback before stock mutation, payables reduction, AP reversal journals, analytics totals, backup inclusion, and frontend schema/detail/analytics rendering.
- Verification for the checkpoint: syntax checks pass for `server/app.js`, `server/ledger.js`, and `server/accounting.js`; focused backend purchase/ledger/payables regression passes (`node --test test/purchase.test.js test/ledger.test.js test/accounting-payables.test.js`, 28 pass, 0 fail); focused modern schema/Purchase regression passes (`npm --prefix web-modern test -- src/lib/api/schemas.test.ts src/routes/app/purchase/-orderId.test.tsx src/routes/app/purchase/-index.test.tsx`, 121 pass, 0 fail); full backend test suite passes (`npm test`, 1505 pass, 0 fail); full modern web test suite passes (`npm --prefix web-modern run test`, 2589 pass, 0 fail); `npm --prefix web-modern run typecheck` passes; `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; `git diff --check` passes; direct egress contract check passes (`node scripts/check-egress-policy-contract.mjs`, `failing_checks=0`); `ARMOSPHERA_ONE_DB=/tmp/a1-suite-purchase-credit-note-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=15, kpis=4`).

### Slice 416 - Purchase Landed-Cost Evidence Before Receipt
- Landed-cost allocations now persist both header evidence (`landed_cost_allocations`) and durable per-line allocation rows (`landed_cost_lines`) with amount, basis, quantity, subtotal, and unit-cost-delta evidence; startup repair creates the child table/index and adds `allocation_json` for older allocation headers.
- The procurement landed-cost engine now allocates exact integer shares by value, quantity, or weight, preserves the commercial PO line `unit_cost`, rejects landed costs after receipt/billing, and returns both the new `allocated` shape and legacy `allocations` aliases.
- Purchase now owns a first-class `POST /api/purchase/orders/:id/landed-costs` route with scoped idempotency replay, and `GET /api/purchase/orders/:id` returns the enriched order detail used by the modern Purchase detail page.
- Purchase order and analytics payloads now expose landed-cost count/amount, while order lines expose `landedCostAmount`, `landedUnitCostDelta`, `effectiveUnitCost`, and child landed-cost evidence; receipt stock moves use the landed-inclusive effective unit cost without rewriting the original PO line unit cost.
- Admin backups now include `landed_cost_allocations` and `landed_cost_lines`, and the modern Purchase detail page renders a read-only landed-cost evidence panel beside billed-return credit-note evidence.
- The ERP comparison plan now marks pre-receipt landed-cost allocation evidence as shipped while leaving post-receipt revaluation/accounting, tenders/blanket-order depth, replenishment, valuation accounting, lots/serials, and advanced WMS as remaining scope.
- Verification for the checkpoint: syntax checks pass for `server/app.js`, `server/procurement.js`, and `server/db.js`; focused backend procurement regression passes (`node --test test/procurement-extension.test.js --test-name-pattern "landed"`, 13 pass, 0 fail); focused backend migration/API regression passes inside the larger API run (`node --test test/procurement-extension.test.js test/api.test.js --test-name-pattern "landed|purchase/order landed|existing landed"`, 249 pass, 0 fail); focused modern schema/Purchase detail regression passes (`npm --prefix web-modern test -- src/lib/api/schemas.test.ts src/routes/app/purchase/-orderId.test.tsx`, 102 pass, 0 fail); full backend test suite passes (`npm test`, 1507 pass, 0 fail); full modern web test suite passes (`npm --prefix web-modern run test`, 2592 pass, 0 fail); `npm --prefix web-modern run typecheck` passes; `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; `git diff --check` passes; direct egress contract check passes (`node scripts/check-egress-policy-contract.mjs`, `failing_checks=0`); `ARMOSPHERA_ONE_DB=/tmp/a1-suite-landed-cost-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=15, kpis=4`).

### Slice 417 - Purchase Replenishment Demand Suggestions
- Procurement replenishment analytics now score active stock-tracked catalog items from real available stock, reserved quantity, open RFQ/confirmed/partial PO cover, active sales quote demand, recent customer issue velocity, vendor price validity, and lead-time evidence instead of only flagging zero-stock/no-PO rows.
- `GET /api/procurement/analytics/replenishment` now returns a summary plus deterministic suggestion rows with sales-demand, open-PO, safety-stock, net-available, demand-source, vendor, lead-time, driver, and reasoning evidence; Purchase analytics embeds the same summary and queue for dashboard use.
- The modern Procurement workspace now includes a sixth `Replenishment` tab with a read-only demand queue, inventory links, empty/loading/error states, and hash-aware deep links so `/app/purchase/procurement#replenishment` opens the queue directly.
- The modern Purchase analytics page now surfaces a Replenishment KPI and a compact Demand queue preview that links both to inventory stock detail and to the replenishment procurement tab.
- The frontend API schemas share one replenishment suggestion contract across Purchase analytics and Procurement analytics, avoiding route-specific parse drift as the backend payload evolves.
- The ERP comparison plan now marks first purchase-to-sales replenishment suggestions as shipped while leaving tenders/blanket-order depth, advanced vendor lifecycle, post-receipt landed-cost revaluation/accounting, lots/serials, valuation accounting, and advanced WMS as remaining scope.
- Verification for the checkpoint: syntax checks pass for `server/procurement.js` and `server/app.js`; focused backend replenishment regression passes (`node --test test/procurement-extension.test.js --test-name-pattern replenishment`, 13 pass, 0 fail); focused modern schema/Purchase/Procurement route regression passes (`npm --prefix web-modern test -- src/routes/app/purchase/-index.test.tsx src/routes/app/purchase/procurement/-index.test.tsx src/lib/api/schemas.test.ts`, 142 pass, 0 fail); deterministic vault tamper regression passes (`node --test server/lib/__tests__/vault.test.js --test-name-pattern "tampered ciphertext"`, 34 pass, 0 fail); full backend test suite passes (`npm test`, 1507 pass, 0 fail); full modern web test suite passes (`npm --prefix web-modern run test`, 2601 pass, 0 fail); `npm --prefix web-modern run typecheck` passes; `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; `git diff --check` passes; direct egress contract check passes (`node scripts/check-egress-policy-contract.mjs`, `failing_checks=0`); `ARMOSPHERA_ONE_DB=/tmp/a1-suite-replenishment-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=15, kpis=4`).

### Slice 418 - Procurement Blanket Agreement Coverage
- Blanket orders now return a full read model instead of an id-only acknowledgement, including vendor/item labels, agreement dates, committed quantity, consumed quantity, remaining quantity, unit price, currency, UoM, status, and note evidence.
- Blanket coverage now filters to active agreement windows, excludes expired/future agreements, validates active vendors and catalog items on create, calculates open PO consumption, and reports committed, remaining, and uncovered quantities for the requested catalog item.
- `POST /api/procurement/blanket-orders` now returns both canonical `blanket` and legacy `blanketOrder` aliases for idempotent callers, while `GET /api/procurement/blanket-orders/coverage?productId=...` returns enriched blanket rows instead of only a count.
- The modern Procurement workspace now includes a seventh `Blanket` tab with blanket-order creation, coverage lookup by catalog item, loading/empty/error states, hash-aware deep links, and a coverage table showing vendor, item, committed, consumed, remaining, open PO, and uncovered evidence.
- The frontend API schemas accept the enriched blanket-order and coverage payloads, and route tests cover create, coverage lookup, empty/error/loading states, and `#blanket` navigation.
- The ERP comparison plan now marks first blanket agreement coverage evidence as shipped while keeping tenders, advanced vendor lifecycle/pricelist depth, post-receipt landed-cost revaluation/accounting, lots/serials, valuation accounting, and advanced WMS as remaining scope.
- Verification for the checkpoint: syntax checks pass for `server/procurement.js` and `server/app.js`; focused backend blanket regression passes (`node --test test/procurement-extension.test.js --test-name-pattern "blanket"`, 13 pass, 0 fail); focused modern schema/Procurement route regression passes (`npm --prefix web-modern test -- src/routes/app/purchase/procurement/-index.test.tsx src/lib/api/schemas.test.ts src/lib/procurement/__tests__/status.test.ts`, 172 pass, 0 fail); full backend test suite passes (`npm test`, 1507 pass, 0 fail); full modern web test suite passes (`npm --prefix web-modern run test`, 2611 pass, 0 fail); `npm --prefix web-modern run typecheck` passes; `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; `git diff --check` passes; direct egress contract check passes (`node scripts/check-egress-policy-contract.mjs`, `failing_checks=0`); `ARMOSPHERA_ONE_DB=/tmp/a1-suite-blanket-coverage-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=15, kpis=4`).

### Slice 419 - Vendor Lifecycle And Pricelist Governance
- Purchase vendors now carry first-class pricelist lifecycle evidence, including usable, expired, future, archived, and expiring-soon price counts, next-expiry evidence, risk levels, and concise risk reasons for operators.
- Purchase analytics now summarizes vendor lifecycle risk across active, inactive, and blocked vendors so the Purchase workspace can show expiring-price and stale-price exposure alongside order and replenishment health.
- RFQ supplier scoring, AI supplier selection, and replenishment recommendations now ignore inactive vendors and unusable price rows, honoring active status, Armenia-local validity windows, and minimum quantity thresholds before recommending a vendor.
- The modern Purchase workspace now exposes the vendor price-risk signal directly in the Vendors table and adds a compact lifecycle-risk panel to Analytics, keeping tender workflows and post-receipt landed-cost accounting as remaining procurement scope.
- Verification for the checkpoint: syntax checks pass for `server/app.js` and `server/procurement.js`; focused backend Purchase/Procurement regression passes (`node --test test/purchase.test.js test/procurement-extension.test.js`, 23 pass, 0 fail); focused modern schema/status/Purchase route regression passes (`npm --prefix web-modern run test -- src/lib/api/schemas.test.ts src/lib/purchase/__tests__/status.test.ts src/routes/app/purchase/-index.test.tsx`, 159 pass, 0 fail); full backend test suite passes (`npm test`, 1509 pass, 0 fail); full modern web test suite passes (`npm --prefix web-modern run test`, 2621 pass, 0 fail); `npm --prefix web-modern run typecheck` passes; `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; `git diff --check` passes; direct egress contract check passes (`node scripts/check-egress-policy-contract.mjs`, `failing_checks=0`); `ARMOSPHERA_ONE_DB=/tmp/a1-suite-vendor-lifecycle-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=15, kpis=4`).

### Slice 420 - POS Cash Session Fiscal Closeout Spine
- The suite now has a first POS app entry in the app catalog, role assignments, legacy route registry, modern navigation, and `/app/pos` route so retail operators can open the POS workspace from the local suite.
- Backend POS storage now includes tenant-scoped `pos_cash_sessions` with cashier, stock location, register, opening cash, expected cash, counted cash, difference, fiscal device, Z-report, receipt range, close note, audit metadata, and tenant-backup coverage.
- `GET /api/pos/workspace`, `GET /api/pos/cash-sessions`, `POST /api/pos/cash-sessions`, and `POST /api/pos/cash-sessions/:id/close` provide the bounded cash-session spine: open a drawer, list recent sessions, and close with Z-report-like fiscal evidence. At this checkpoint sale, refund, stock, receipt-printing, offline replay, and ledger posting were still marked as not implemented; Slice 422 adds the first sale/stock posting path.
- The modern POS workspace shows open-session controls, current-session closeout, fiscal evidence labels, recent sessions, fiscal catalog items, and active internal stock locations without claiming full POS sales or accounting automation.
- The ERP comparison plan now marks first Armenian POS cash-session closeout evidence as shipped while leaving POS sale posting, refunds, offline replay, receipt printing, cash/card split, inventory decrement, and revenue/VAT/cash ledger posting as remaining Phase 2 scope at this checkpoint; Slice 422 narrows that remaining list.
- Verification for the checkpoint: focused syntax checks pass for `server/app.js` and `server/db.js`; focused backend POS regression passes (`node --test test/pos.test.js`, 4 pass, 0 fail); focused modern schema/app/POS route regression passes (`npm exec -- vitest run src/lib/api/schemas.test.ts src/lib/apps.test.ts src/routes/app/pos/-index.test.tsx --maxWorkers=1`, 118 pass, 0 fail); backend API regression passes at reduced concurrency (`node --test --test-concurrency=1 --test-timeout=180000 test/api.test.js`, 236 pass, 0 fail); remaining backend regression passes (`node --test --test-concurrency=4 --test-timeout=180000 <all non-api backend test files>`, 1277 pass, 0 fail); full modern web test suite passes (`npm --prefix web-modern run test`, 2634 pass, 0 fail); `npm --prefix web-modern run typecheck` passes; `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; `git diff --check` passes; direct egress contract check passes (`node scripts/check-egress-policy-contract.mjs`, `failing_checks=0`); `ARMOSPHERA_ONE_DB=/tmp/a1-suite-pos-cash-session-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=16, kpis=4`).

### Slice 421 - Procurement RFQ Tender Contract Alignment
- Procurement requisition and RFQ responses now have a stable tender contract: requisition lines include `uom`, RFQs expose `createdAt`, `quotes`, `quoteCount`, `award`, stable shortlisted vendor labels, and compatibility aliases for modern UI consumers.
- RFQ quote capture now returns vendor and requisition-line evidence, while RFQ award creates a draft PO with real subtotal/total, currency, selected quote IDs, line evidence, and award evidence instead of the old zero-total shell.
- The modern Procurement workspace now follows the live backend chain: create a requisition with a non-empty line, convert it through `/api/procurement/requisitions/:id/convert-to-rfq`, record quotes through `/api/procurement/rfqs/:id/quotes`, award through `/api/procurement/rfqs/:id/award`, and keep receiving deferred to the existing purchase-order receiving flow.
- Frontend API schemas, route tests, and the Playwright procurement spec now model the nested RFQ tender contract instead of the former flat mock-only endpoints.
- The ERP comparison plan now marks first RFQ tender contract alignment as shipped while leaving advanced tender packages, supplier portal/bid intake, comparative award matrices, receiving depth, and post-receipt landed-cost revaluation/accounting as remaining procurement scope.
- Verification for the checkpoint: syntax checks pass for `server/procurement.js` and `server/app.js`; focused backend procurement regression passes (`node --test test/procurement-extension.test.js`, 14 pass, 0 fail); focused modern schema/Procurement route regression passes (`npm --prefix web-modern run test -- src/routes/app/purchase/procurement/-index.test.tsx src/lib/api/schemas.test.ts`, 142 pass, 0 fail); full backend test suite passes (`npm test`, 1513 pass, 0 fail); full modern web test suite passes (`npm --prefix web-modern run test`, 2638 pass, 0 fail); `npm --prefix web-modern run typecheck` passes; `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; `git diff --check` passes; direct egress contract check passes (`node scripts/check-egress-policy-contract.mjs`, `failing_checks=0`); `ARMOSPHERA_ONE_DB=/tmp/a1-suite-rfq-contract-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=16, kpis=4`).

### Slice 422 - POS Cash Sale Posting And Stock Decrement
- Backend POS storage now includes tenant-scoped `pos_sales` and `pos_sale_lines` with source-key uniqueness, per-session receipt uniqueness, line-level catalog evidence, posting status fields, audit metadata, and tenant-backup coverage.
- `POST /api/pos/cash-sessions/:id/sales` now validates an open cash session, active fiscal AMD catalog items, positive whole quantities, receipt/source metadata, stock availability, and same-session source-key replay before posting durable sale evidence.
- Cash sale posting computes VAT-inclusive subtotal/VAT/total from catalog list prices, updates session `expected_cash_amd` for cash payments, returns the refreshed session, and rejects duplicate receipt numbers without double-posting stock or cash.
- Stock-tracked fiscal sale lines now create canonical `delivery` stock moves from the session stock location to the customer sink, link each posted line to its `stockMoveId`, and expose inventory posting evidence in the sale/session responses.
- The modern POS workspace now adds a one-line Sale Capture panel for open sessions, posting selected fiscal catalog item, quantity, receipt number, payment method, optional sold-at evidence, loading/error/success state, and workspace invalidation after the sale.
- POS capability flags now mark first sale/inventory posting as available while keeping ledger posting, refunds/voids, offline replay, receipt printing/fiscal-device submission, payment split accounting, lots/serials, barcode depth, and revenue/VAT/cash/bank journals deferred.
- The ERP comparison plan now marks first POS cash sale posting and stock decrement as shipped while leaving the deeper fiscal-device, offline, refund, payment, and accounting integrations in Phase 2.
- Verification for the checkpoint: syntax checks pass for `server/app.js` and `server/db.js`; focused backend POS regression passes (`node --test test/pos.test.js`, 7 pass, 0 fail); focused modern POS/schema regression passes (`npm --prefix web-modern run test -- src/routes/app/pos/-index.test.tsx src/lib/api/schemas.test.ts`, 110 pass, 0 fail); full backend test suite passes (`npm test`, 1516 pass, 0 fail); full modern web test suite passes (`npm --prefix web-modern run test`, 2643 pass, 0 fail); `npm --prefix web-modern run typecheck` passes; `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; `git diff --check` passes; direct egress contract check passes (`node scripts/check-egress-policy-contract.mjs`, `failing_checks=0`); `ARMOSPHERA_ONE_DB=/tmp/a1-suite-pos-sale-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=16, kpis=4`).

### Slice 423 - POS Fiscal Receipt Handoff Packet Evidence
- Backend POS storage now includes tenant-scoped `pos_receipt_packets` with sale/session/receipt references, fiscal device evidence, prepared-only packet status, packet kind/format, SHA-256 checksum, payload JSON, prepared-by metadata, and tenant-backup coverage.
- `POST /api/pos/sales/:id/receipt-packet` now validates POS app/write access, safe sale IDs, posted sale state, fiscal-device evidence from the request or sale session, and idempotent replay by sale before preparing local fiscal receipt handoff evidence.
- Receipt packet payloads include sale totals, payment method, session cash/fiscal context, sale lines, posting statuses, fiscal device ID, receipt number, explicit `not-submitted` status, and controls stating that the packet is local handoff evidence only.
- The modern POS workspace now lets the operator prepare receipt evidence for the last posted sale and renders the prepared status plus checksum without claiming live printing, fiscal-device submission, or legal fiscalization.
- Frontend API schemas now parse the receipt-packet request/response contract beside the existing sale contract, keeping POS route tests aligned to `/api/pos/sales/:id/receipt-packet`.
- The ERP comparison plan now marks first local fiscal receipt handoff evidence as shipped while keeping live fiscal-device submission, receipt printing, refunds/voids, offline replay, payment split accounting, and revenue/VAT/cash/bank ledger posting deferred.
- Verification for the checkpoint: syntax checks pass for `server/app.js` and `server/db.js`; focused backend POS regression passes (`node --test test/pos.test.js`, 9 pass, 0 fail); focused modern POS/schema regression passes (`npm --prefix web-modern run test -- src/routes/app/pos/-index.test.tsx src/lib/api/schemas.test.ts`, 114 pass, 0 fail); full backend test suite passes (`npm test`, 1518 pass, 0 fail); full modern web test suite passes (`npm --prefix web-modern run test`, 2647 pass, 0 fail); `npm --prefix web-modern run typecheck` passes; `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; `git diff --check` passes; direct egress contract check passes (`node scripts/check-egress-policy-contract.mjs`, `failing_checks=0`); `ARMOSPHERA_ONE_DB=/tmp/a1-suite-pos-receipt-packet-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=16, kpis=4`).

### Slice 424 - POS Full-Sale Refund Evidence
- Backend POS storage now includes tenant-scoped `pos_sale_refunds` and `pos_sale_refund_lines` with sale/session references, refund reference, source-key replay, reason, method, full-sale totals, cash adjustment evidence, posting statuses, created-by metadata, and tenant-backup coverage.
- `POST /api/pos/sales/:id/refund` validates POS app/write access, safe sale IDs, posted sale state, safe refund metadata, source-key idempotency, single-refund-per-sale protection, and optional refunded-at evidence before recording the refund.
- Cash full-sale refund evidence reduces an open cash session's expected cash by the original paid-cash amount, while card and bank-transfer refund evidence leaves drawer cash unchanged.
- Refund posting marks the original sale `refunded_full`, preserves original sale posting evidence, copies original sale-line and source stock-move evidence into refund lines, and left tracked-line inventory restock for the next slice.
- The modern POS workspace lets the operator record refund evidence for the last posted sale, posts to `/api/pos/sales/:id/refund`, renders refund status, total, method, cash adjustment, inventory/ledger evidence, refreshes the open session, and locks duplicate refund entry after success.
- Frontend API schemas now parse the refund request/response contract and accept refunded POS sale statuses beside the existing posted sale contract.
- The ERP comparison plan marked first POS full-sale refund evidence as shipped while keeping voids, partial refunds, tracked-line restock until Slice 425, terminal/fiscal submission, payment split accounting, offline replay, and revenue/VAT/cash/bank ledger journals deferred.
- Verification for the checkpoint: syntax checks pass for `server/app.js` and `server/db.js`; focused backend POS regression passes (`node --test test/pos.test.js`, 13 pass, 0 fail); focused Projects/Inventory/Service regression passes (`node --test --test-concurrency=4 --test-timeout=60000 test/projects.test.js test/project-billing.test.js test/inventory.test.js test/service-cases.test.js`, 56 pass, 0 fail); focused Procurement and Purchase regressions pass (`test/procurement-extension.test.js`, 14 pass, 0 fail; `test/purchase.test.js`, 9 pass, 0 fail); full backend suite passes (`npm test`, 1653 pass, 0 fail); full modern web suite passes (`npm --prefix web-modern run test`, 2707 pass, 0 fail); `npm --prefix web-modern run typecheck` passes; `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; `git diff --check` passes; direct egress contract check passes (`node scripts/check-egress-policy-contract.mjs`, `failing_checks=0`); `ARMOSPHERA_ONE_DB=/tmp/a1-suite-pos-refund-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=16, kpis=4`).

### Slice 425 - POS Refund Stock Return Evidence
- Backend POS refund storage now adds `pos_sale_refund_lines.return_stock_move_id` with repair-time migration coverage for existing local databases.
- `POST /api/pos/sales/:id/refund` now creates a linked customer-to-internal `return` stock move for every tracked original sale line, stores that move on the refund line, marks refund inventory posting as `posted`, and preserves source-key idempotency so replay does not double-restock.
- Cash refunds still reduce only open-session drawer cash; card and bank-transfer refunds restore tracked inventory without changing expected cash. Fiscal refund submission, receipt printing, terminal submission, and revenue/VAT/cash/bank ledger journals remain deferred.
- The modern POS workspace now renders refund inventory as posted when return moves exist, shows the return stock move count, and keeps ledger/fiscal/receipt-printing deferrals explicit.
- Frontend API schemas now parse `returnStockMoveId` on refund lines, and POS route tests assert the inventory-posted refund evidence path.
- The ERP comparison plan now marks full-sale tracked refund return stock flow as shipped while keeping customer-linked refunds, voids, partial refunds, offline replay, live fiscal-device submission, payment split accounting, lots/serials/barcode depth, and balanced accounting journals deferred.
- Verification for the checkpoint: syntax checks pass for `server/app.js` and `server/db.js`; focused backend POS regression passes (`node --test test/pos.test.js`, 13 pass, 0 fail); focused Inventory/Purchase regression passes (`node --test --test-concurrency=4 --test-timeout=60000 test/inventory.test.js test/purchase.test.js`, 15 pass, 0 fail); focused modern POS/schema regression passes (`npm --prefix web-modern run test -- src/routes/app/pos/-index.test.tsx src/lib/api/schemas.test.ts`, 118 pass, 0 fail); full backend suite passes (`npm test`, 1653 pass, 0 fail); full modern web suite passes (`npm --prefix web-modern run test`, 2707 pass, 0 fail); `npm --prefix web-modern run typecheck` passes; `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; `git diff --check` passes; direct egress contract check passes (`node scripts/check-egress-policy-contract.mjs`, `failing_checks=0`); `ARMOSPHERA_ONE_DB=/tmp/a1-suite-pos-refund-return-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=16, kpis=4`).

### Slice 426 - POS Sale And Refund Ledger Posting
- POS sales now post balanced revenue, output VAT, and settlement journals through the shared ledger helper for cash (`251`), bank transfer (`252`), and card clearing (`255`) payment methods while preserving integer AMD storage and period-lock rollback behavior from `ledger.postEntry`.
- POS full-sale refunds now post reversal journals against revenue and output VAT, crediting the matching cash/bank/card settlement account and allowing VAT reports to net debit-side output VAT reversals correctly.
- POS sale and refund API read models now expose `postings.ledgerPostingIds` and `postings.ledgerPostingCount`, while cash sessions report ledger posting state from posted sale/refund evidence.
- Tenant backups now include `ledger_journal` so POS sale/refund accounting evidence is restorable alongside POS sale, refund, stock, audit, and session evidence.
- The modern POS workspace now shows sale ledger status and journal count in the sale success row, shows refund ledger journal count in refund evidence, and exposes the current session ledger posting state in the fiscal closeout panel.
- Frontend API schemas now type POS session posting evidence and POS sale/refund ledger posting ID/count fields, with schema and route tests covering the posted journal evidence path.
- The ERP comparison plan now marks first POS sale/refund revenue-VAT-settlement ledger journals as shipped while keeping offline replay, voids/partial refunds, live fiscal-device submission, receipt printing, payment split accounting, terminal settlement reconciliation, lots/serials, loyalty, and deeper barcode operations as remaining POS depth.
- Verification for the checkpoint: syntax checks pass for `server/app.js` and `server/ledger.js`; focused backend POS regression passes (`node --test test/pos.test.js`, 13 pass, 0 fail); focused modern POS/schema regression passes (`npm --prefix web-modern run test -- src/routes/app/pos/-index.test.tsx src/lib/api/schemas.test.ts`, 118 pass, 0 fail); focused ledger/VAT regression passes (`node --test test/ledger.test.js test/finance-reports.test.js`, 18 pass, 0 fail); focused API/Purchase regression passes (`node --test --test-concurrency=4 --test-timeout=60000 test/purchase.test.js test/api.test.js`, 245 pass, 0 fail); full backend suite passes (`npm test`, 1653 pass, 0 fail); full modern web suite passes (`npm --prefix web-modern run test`, 2707 pass, 0 fail); `npm --prefix web-modern run typecheck` passes; `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; `git diff --check` passes; direct egress contract check passes (`node scripts/check-egress-policy-contract.mjs`, `failing_checks=0`); `ARMOSPHERA_ONE_DB=/tmp/a1-suite-pos-ledger-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=16, kpis=4`).

### Slice 427 - POS Terminal Settlement Evidence
- Backend POS storage now includes tenant-scoped `pos_terminal_settlements` with closed cash-session references, terminal batch reference, source-key replay, provider evidence, card clearing/bank account codes, expected and settled AMD totals, difference evidence, posting status, operator metadata, repair-time index/column coverage for existing local databases, and tenant-backup coverage.
- `GET /api/pos/cash-sessions/:id/terminal-settlement-preview` now returns closed-session card sales, card refunds, prior posted settlements, net card clearing, outstanding amount, readiness, account codes, and recent settlement evidence.
- `POST /api/pos/cash-sessions/:id/terminal-settlements` validates POS writer access, safe metadata, closed-session state, positive outstanding card clearing, same-session idempotency, and bounded settled totals before posting settlement evidence.
- Terminal settlement posting creates a balanced ledger journal from card clearing (`255`) to bank (`252`) for the settled amount, exposing `postings.ledgerPostingIds` and `postings.ledgerPostingCount` on the settlement read model.
- The POS workspace response now includes terminal settlement previews for recent closed sessions and marks terminal settlement capability as available while keeping offline replay, live fiscal submission, receipt printing, voids, partial refunds, payment split accounting, live terminal feeds, processor fees, and chargebacks deferred.
- The modern POS workspace now shows a terminal settlement panel beside fiscal closeout: operators can inspect card clearing totals for closed sessions, post settlement reference/provider/amount/date/note evidence, and see clearing account, bank account, difference, and ledger journal count after posting.
- Frontend API schemas now type terminal settlement request/response/preview payloads, and route tests cover posting to `/api/pos/cash-sessions/:id/terminal-settlements` with internal idempotency and rendered ledger evidence.
- The ERP comparison plan now marks first closed-session card terminal settlement evidence as shipped while keeping live terminal provider feeds, processor-fee reconciliation, chargebacks, offline/live-fiscal-device handling, richer returns, lots/serials, loyalty, and deep barcode operations as remaining POS depth.
- Verification for the checkpoint: syntax checks pass for `server/app.js`, `server/db.js`, and `server/ledger.js`; focused backend POS regression passes (`node --test test/pos.test.js`, 14 pass, 0 fail); focused ledger/VAT regression passes (`node --test test/ledger.test.js test/finance-reports.test.js`, 18 pass, 0 fail); focused modern POS/schema regression passes (`npm --prefix web-modern run test -- src/lib/api/schemas.test.ts src/routes/app/pos/-index.test.tsx`, 122 pass, 0 fail); full backend suite passes (`npm test`, 1654 pass, 0 fail); full modern web suite passes (`npm --prefix web-modern run test`, 2711 pass, 0 fail); `npm --prefix web-modern run typecheck` passes; `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; `git diff --check` passes; direct egress contract check passes (`node scripts/check-egress-policy-contract.mjs`, `failing_checks=0`); `ARMOSPHERA_ONE_DB=/tmp/a1-suite-pos-terminal-settlement-smoke-post-repair.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=16, kpis=4`); local dev backend on `localhost:4100` returns 401 for the terminal-settlement preview route when unauthenticated, confirming the route is present and auth-gated.

### Slice 428 - POS Terminal Processor Fee Evidence
- POS terminal settlement storage now carries `processor_fee_amd` and `fee_account_code` evidence with repair-time coverage for existing local databases and tenant-backup inclusion.
- Closed-session terminal settlement preview now reduces outstanding card clearing by prior bank deposits plus prior processor fees, exposes processor-fee totals, cleared totals, and the fee expense account, and keeps no-fee settlements backward compatible.
- `POST /api/pos/cash-sessions/:id/terminal-settlements` now accepts optional processor fee evidence, rejects over-clearing where bank deposit plus fee exceeds outstanding card clearing, and stores the cleared/difference evidence on the settlement.
- Ledger posting now creates the original card-clearing-to-bank journal for the deposited amount and, when a processor fee is present, a second card-clearing-to-expense journal (`711`) for the fee amount.
- The modern POS terminal settlement panel now includes processor-fee entry, live cleared-total/difference/outstanding calculations, and posted evidence for bank deposit, processor fee account, total cleared, remaining outstanding amount, and fee journal count.
- Frontend API schemas and POS route tests now cover fee-bearing terminal settlement requests/responses while preserving additive parsing for older no-fee settlement rows.
- The ERP comparison plan now marks first processor-fee reconciliation evidence as shipped while keeping live terminal provider feeds, chargebacks, offline/live-fiscal-device handling, richer returns, lots/serials, loyalty, and deep barcode operations as remaining POS depth.
- Verification for the checkpoint: syntax checks pass for `server/app.js`, `server/db.js`, and `server/ledger.js`; focused backend POS regression passes (`node --test test/pos.test.js`, 14 pass, 0 fail); focused modern POS/schema regression passes (`npm --prefix web-modern run test -- src/lib/api/schemas.test.ts src/routes/app/pos/-index.test.tsx`, 122 pass, 0 fail); focused ledger/VAT regression passes (`node --test test/ledger.test.js test/finance-reports.test.js`, 18 pass, 0 fail); full backend suite passes (`npm test`, 1654 pass, 0 fail); full modern web suite passes (`npm --prefix web-modern run test`, 2711 pass, 0 fail); `npm --prefix web-modern run typecheck` passes; `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; `git diff --check` passes; direct egress contract check passes (`node scripts/check-egress-policy-contract.mjs`, `failing_checks=0`); `ARMOSPHERA_ONE_DB=/tmp/a1-suite-pos-terminal-fee-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=16, kpis=4`).

### Slice 429 - POS Single Partial Refund Evidence
- `POST /api/pos/sales/:id/refund` now accepts a positive `refundedTotal` up to the original sale total while preserving the existing omitted-total full refund behavior and one-refund-per-sale protection.
- Partial refunds mark the sale `refunded`, reduce open-session expected cash only by the partial cash refund amount, and leave card/bank refund drawer cash unchanged through the existing refund-method handling.
- POS refund ledger posting now reverses revenue and output VAT by the proportional partial amount derived from the original sale subtotal, VAT, and total, so partial refunds do not over-reverse posted sale revenue or VAT.
- Partial refund lines now preserve proportional subtotal/VAT/total evidence against the original sale line while deliberately leaving `returnStockMoveId` empty and reporting `inventoryPostingStatus: "not-posted"` until line-level partial stock-return controls are implemented.
- The modern POS refund form now accepts an optional amount field, keeps blank-as-full refund behavior, sends `refundedTotal` for partial refunds, and renders returned `refunded` sale status, refunded total, cash adjustment, inventory status, return-stock-move count, and no-stock-return copy.
- The ERP comparison plan now marks first single partial refund amount evidence as shipped while keeping voids, customer-linked refunds, line-level partial stock returns, offline/live-fiscal-device handling, payment split accounting, lots/serials, loyalty, live terminal feeds, chargebacks, and deep barcode operations as remaining POS depth.
- Verification for the checkpoint: syntax checks pass for `server/app.js` and `server/ledger.js`; `git diff --check` passes; focused backend POS regression passes (`node --test test/pos.test.js`, 15 pass, 0 fail); focused modern POS/schema regression passes (`npm --prefix web-modern run test -- src/lib/api/schemas.test.ts src/routes/app/pos/-index.test.tsx`, 123 pass, 0 fail); focused ledger/VAT regression passes (`node --test test/ledger.test.js test/finance-reports.test.js`, 18 pass, 0 fail); full backend suite passes (`npm test`, 1655 pass, 0 fail); full modern web suite passes (`npm --prefix web-modern run test`, 2712 pass, 0 fail); `npm --prefix web-modern run typecheck` passes; `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; direct egress contract check passes (`node scripts/check-egress-policy-contract.mjs`, `failing_checks=0`); `ARMOSPHERA_ONE_DB=/tmp/a1-suite-pos-partial-refund-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=16, kpis=4`).

### Slice 430 - POS Payment Split Accounting Evidence
- POS sale storage now includes tenant-scoped `pos_sale_payments` rows with local database repair coverage, backup inclusion, and legacy single-method fallback for older sale rows.
- `POST /api/pos/cash-sessions/:id/sales` now accepts optional cash/card/bank-transfer payment splits, validates positive whole-AMD split amounts, rejects mismatched totals, stores payment evidence, and keeps the legacy `paymentMethod` contract for old clients.
- Cash-session expected cash now increases only by the cash portion of a split sale, while card portions remain available to closed-session terminal-settlement preview and processor-fee settlement flows.
- POS sale ledger posting now splits revenue and output VAT across cash, bank-transfer, and card-clearing settlement accounts using the payment evidence, while preserving balanced single-method posting behavior.
- Local fiscal receipt packet payloads now include payment rows and payment totals so the offline handoff evidence matches the recorded sale settlement evidence.
- The modern POS sale capture panel now has compact split-payment inputs, client-side whole-AMD/sum validation, and post-sale payment evidence for method count plus cash/card/bank-transfer totals.
- The ERP comparison plan now marks first payment split accounting evidence as shipped while keeping offline replay, voids, line-level partial stock returns, live fiscal-device submission, receipt printing, lots/serials, loyalty, live terminal feeds, chargebacks, and deep barcode operations as remaining POS depth.
- Verification for the checkpoint: syntax checks pass for `server/app.js`, `server/db.js`, `server/ledger.js`, and `test/pos.test.js`; `git diff --check` passes; focused backend POS regression passes (`node --test test/pos.test.js`, 16 pass, 0 fail); focused ledger/finance regression passes (`node --test test/ledger.test.js test/finance-posting.test.js test/finance-reports.test.js`, 20 pass, 0 fail); focused modern POS/schema regression passes (`npm --prefix web-modern run test -- src/lib/api/schemas.test.ts src/routes/app/pos/-index.test.tsx`, 124 pass, 0 fail); full backend suite passes (`npm test`, 1656 pass, 0 fail); full modern web suite passes (`npm --prefix web-modern run test`, 2713 pass, 0 fail); `npm --prefix web-modern run typecheck` passes; `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; direct egress contract check passes (`node scripts/check-egress-policy-contract.mjs`, `failing_checks=0`); `ARMOSPHERA_ONE_DB=/tmp/a1-suite-pos-payment-split-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=16, kpis=4`).

### Slice 431 - POS Pre-Receipt Void Evidence
- POS void storage now includes tenant-scoped `pos_sale_voids` and `pos_sale_void_lines` with sale/session references, void reference, source-key replay, reason, voided total, cash adjustment, inventory/ledger posting statuses, operator metadata, startup repair coverage for existing local databases, and tenant-backup inclusion.
- `POST /api/pos/sales/:id/void` validates POS writer access, safe sale IDs and metadata, posted sale state, open cash-session state, no prior receipt packet, source-key idempotency, and single-void-per-sale protection before recording the void evidence.
- Pre-receipt cash voids reduce the open cash-session expected cash by the original paid-cash amount, create tracked-line customer-to-internal return stock moves, mark the sale `voided`, and preserve idempotent replay without duplicate cash, stock, audit, event, or ledger side effects.
- POS void ledger posting now reverses revenue and output VAT against the original sale settlement account, with split-payment evidence preserved through the shared POS ledger helper.
- The modern POS workspace now renders a void evidence form for the last posted sale, posts to `/api/pos/sales/:id/void`, shows voided total, cash adjustment, inventory status, return stock move count, and ledger journal count, and locks receipt/refund entry once the sale is voided.
- The ERP comparison plan now marks first pre-receipt void evidence as shipped while keeping offline replay, line-level partial stock returns, live fiscal-device submission, receipt printing, lots/serials, loyalty, live terminal feeds, chargebacks, and deep barcode operations as remaining POS depth.
- Verification for the checkpoint: syntax checks pass for `server/app.js`, `server/db.js`, `server/ledger.js`, and `test/pos.test.js`; `git diff --check` passes; focused backend POS regression passes (`node --test test/pos.test.js`, 17 pass, 0 fail); focused ledger/finance regression passes (`node --test test/ledger.test.js test/finance-posting.test.js test/finance-reports.test.js`, 20 pass, 0 fail); focused modern POS/schema regression passes (`npm --prefix web-modern run test -- src/lib/api/schemas.test.ts src/routes/app/pos/-index.test.tsx`, 125 pass, 0 fail); full backend suite passes (`npm test`, 1657 pass, 0 fail); full modern web suite passes (`npm --prefix web-modern run test`, 2714 pass, 0 fail); `npm --prefix web-modern run typecheck` passes; `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; `ARMOSPHERA_ONE_DB=/tmp/a1-suite-pos-void-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=16, kpis=4`).

### Slice 432 - POS Line-Level Partial Refund Stock Return Evidence
- `POST /api/pos/sales/:id/refund` now accepts optional `lines: [{ saleLineId, quantity }]` evidence for returned sale-line quantities while preserving the existing omitted-total full refund and amount-only partial refund paths.
- When refund lines are supplied without `refundedTotal`, the backend derives refunded subtotal, VAT, and total from the original sale-line amounts; when both are supplied, the explicit total must match the line-derived total before any cash, stock, or ledger mutation occurs.
- Line-level partial refunds create customer-to-internal return stock moves for the requested quantities, store `returnStockMoveId` on the refund line, restore stock quants by the returned quantity only, mark inventory posting as `posted`, and replay idempotently without duplicate stock moves.
- Amount-only partial refunds remain no-restock goodwill evidence with proportional ledger reversal and `inventoryPostingStatus: "not-posted"`, keeping the existing workflow stable for non-return refunds.
- The modern POS refund form now renders per-line return quantity inputs, previews the line-return total, sends the optional line-return payload, renders returned-line quantity evidence after posting, and keeps duplicate refund entry locked after success.
- Frontend API schemas now validate line-level refund request payloads, and POS route tests cover both amount-only partial refunds and line-return partial refunds from the operator UI.
- The ERP comparison plan now marks line-level partial stock-return refunds as shipped while keeping offline replay, live fiscal-device submission, receipt printing, lots/serials, loyalty, live terminal feeds, chargebacks, and deep barcode operations as remaining POS depth.
- Verification for the checkpoint: syntax checks pass for `server/app.js`, `server/db.js`, `server/ledger.js`, and `test/pos.test.js`; `git diff --check` passes; focused backend POS regression passes (`node --test test/pos.test.js`, 18 pass, 0 fail); focused ledger/finance regression passes (`node --test test/ledger.test.js test/finance-posting.test.js test/finance-reports.test.js`, 20 pass, 0 fail); focused modern POS/schema regression passes (`npm --prefix web-modern run test -- src/lib/api/schemas.test.ts src/routes/app/pos/-index.test.tsx`, 126 pass, 0 fail); `npm --prefix web-modern run typecheck` passes; full backend suite passes (`npm test`, 1658 pass, 0 fail); full modern web suite passes (`npm --prefix web-modern run test`, 2715 pass, 0 fail); `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; Vitest still prints the existing localStorage experimental warning; `ARMOSPHERA_ONE_DB=/tmp/a1-suite-pos-line-refund-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=16, kpis=4`).

### Slice 433 - POS Local Receipt Print Preview Evidence
- POS receipt packet storage now carries local print-preview status, checksum, payload JSON, copy count, operator, timestamp, and repair-time migration coverage for existing local databases.
- `POST /api/pos/sales/:id/receipt-print` validates POS writer access, safe sale IDs and request metadata, requires an already prepared receipt packet, records checksum-backed local print-preview evidence, replays idempotently by sale, and emits audit/suite events without contacting a fiscal device or physical printer.
- Receipt print-preview payloads include the source receipt packet checksum, sale/session/line evidence, preview totals, copy count, explicit `not-submitted` fiscal state, and controls for local-preview-only evidence.
- The modern POS workspace now shows a local print evidence panel after packet preparation, posts to `/api/pos/sales/:id/receipt-print`, renders print status, checksum, not-submitted device state, and compact preview lines while keeping live fiscal/printer submission deferred.
- Frontend API schemas now parse receipt print request/response evidence, including nested packet print evidence for replayed packets.
- The ERP comparison plan now marks first local receipt print-preview evidence as shipped while keeping offline replay, live fiscal-device submission, physical receipt printer commands, lots/serials, loyalty, live terminal feeds, chargebacks, and deep barcode operations as remaining POS depth.
- Verification for the checkpoint: syntax checks pass for `server/app.js`, `server/db.js`, and `test/pos.test.js`; `git diff --check` passes; focused backend POS regression passes (`node --test test/pos.test.js`, 19 pass, 0 fail); focused modern POS/schema regression passes (`npm --prefix web-modern run test -- src/lib/api/schemas.test.ts src/routes/app/pos/-index.test.tsx`, 126 pass, 0 fail); `npm --prefix web-modern run typecheck` passes; full backend suite passes when allowed to bind local loopback test servers (`npm test`, 1659 pass, 0 fail); full modern web suite passes (`npm --prefix web-modern run test`, 2715 pass, 0 fail); `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; Vitest still prints the existing `environmentMatchGlobs` deprecation and localStorage experimental warnings; `ARMOSPHERA_ONE_DB=/tmp/a1-suite-pos-receipt-print-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=16, kpis=4`).

### Slice 434 - POS Offline Replay Readiness Evidence
- POS storage now includes tenant-scoped `pos_offline_replay_items` with cash-session/sale references, action type, replay status, source key, checksum-backed payload JSON, queued/replayed timestamps, operator evidence, startup repair coverage, and tenant-backup inclusion.
- `GET /api/pos/offline-replay-items`, `POST /api/pos/offline-replay-items`, and `POST /api/pos/offline-replay-items/:id/mark-replayed` let POS operators list recent queue evidence, record local offline replay readiness evidence for sale/refund/void/receipt/terminal actions, and mark queued evidence replayed or rejected without executing the underlying POS mutation.
- Queueing validates POS app access, writer role, safe metadata, tenant cash-session ownership, optional sale/session ownership, action type, source-key idempotency, and payload checksum evidence before persistence.
- Marking replay evidence is idempotent for an already-finalized matching status, rejects status changes after finalization, records audit/suite events, and keeps sale, refund, void, stock, receipt packet, terminal settlement, and cash-session state unchanged.
- The modern POS workspace now exposes an offline replay readiness panel with `local-queue` capability evidence, recent queued/replayed/rejected items, sample local queue evidence, and replay/reject marking while explicitly keeping true browser-offline execution, automatic replay, fiscal-device submission, terminal submission, and printer commands deferred.
- Frontend API schemas now parse POS offline replay item/list/queue/mark contracts and the workspace `recentOfflineReplayItems` preview.
- The ERP comparison plan now marks first local offline replay queue readiness as shipped while keeping true browser offline execution, automatic replay/conflict resolution, live fiscal-device submission, physical printer commands, live terminal provider feeds, chargebacks, lots/serials, loyalty, and barcode depth as remaining POS scope.
- Verification for the checkpoint: syntax checks pass for `server/app.js`, `server/db.js`, and `test/pos.test.js`; `git diff --check` passes; focused backend POS regression passes (`node --test --test-timeout=180000 test/pos.test.js`, 21 pass, 0 fail); focused modern POS/schema regression passes (`npm --prefix web-modern run test -- src/lib/api/schemas.test.ts src/routes/app/pos/-index.test.tsx`, 128 pass, 0 fail); full backend suite passes (`npm test`, 1661 pass, 0 fail); full modern web suite passes (`npm --prefix web-modern run test`, 2717 pass, 0 fail); `npm --prefix web-modern run typecheck` passes; `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; Vitest still prints the existing `environmentMatchGlobs` deprecation and localStorage experimental warnings; `ARMOSPHERA_ONE_DB=/tmp/a1-suite-pos-offline-replay-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=16, kpis=4`).

### Slice 435 - POS Customer-Linked Sales And Refunds
- POS sale storage now carries optional `customer_id` with startup repair coverage for existing local databases, customer indexes, and tenant-backup inclusion through the generic backup table loader.
- `POST /api/pos/cash-sessions/:id/sales` and `POST /api/pos/sales` now accept an optional `customerId`, validate it against tenant customers before sale persistence, preserve no-customer sales, and format linked sales with `customerId` and `customerName`.
- POS refund storage now carries inherited `customer_id` evidence from the original sale, including repair-time backfill for existing linked sale/refund rows.
- POS refund responses now format inherited `customerId` and `customerName`, and sale/refund suite events carry linked customer IDs so Customer 360 profile `last_event_at` moves with POS activity.
- `/api/pos/workspace` now exposes a compact customers/customerPreview list for sale selection alongside fiscal catalog and stock-location previews.
- The modern POS workspace now exposes a customer selector in sale capture, posts linked sales with `customerId`, and renders customer evidence on sale and refund result panels while preserving walk-in customer behavior.
- Frontend API schemas now parse POS sale/refund customer evidence and the POS workspace customer picker list.
- Verification for the checkpoint: syntax checks pass for `server/app.js`, `server/db.js`, and `test/pos.test.js`; `git diff --check` passes; focused backend POS regression passes (`node --test --test-timeout=180000 test/pos.test.js`, 23 pass, 0 fail); focused modern POS/schema regression passes (`npm --prefix web-modern run test -- src/lib/api/schemas.test.ts src/routes/app/pos/-index.test.tsx`, 128 pass, 0 fail); full backend suite passes (`npm test`, 1663 pass, 0 fail); full modern web suite passes (`npm --prefix web-modern run test`, 2717 pass, 0 fail); `npm --prefix web-modern run typecheck` passes; `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; Vitest still prints the existing `environmentMatchGlobs` deprecation and localStorage experimental warnings; `ARMOSPHERA_ONE_DB=/tmp/a1-suite-pos-customer-linked-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=16, kpis=4`).

### Slice 436 - POS Browser-Local Sale Draft Queue
- The modern POS sale capture panel now includes a `Queue locally` action that persists the parsed sale payload in browser localStorage with the original sale idempotency key, receipt evidence, customer label, payment label, line label, quantity, and total.
- POS sale posting now automatically records a browser-local draft only for network/offline/backend-unavailable failures, while ordinary business validation errors such as closed finance periods remain visible and are not converted into replayable drafts.
- The POS offline readiness surface now renders a browser sale draft queue with pending draft counts, queued reason, last retry evidence, manual `Retry sale`, and success/removal evidence after the same payload posts successfully.
- The sale draft queue helper validates stored payloads with `PosCreateSaleRequestSchema`, resets corrupt localStorage data, bounds retained drafts to 25, and keeps manual retry separate from automatic replay, fiscal-device submission, terminal submission, and printer commands.
- The ERP comparison plan now marks first browser-local sale draft storage/manual retry as shipped while keeping automatic replay/conflict resolution, live fiscal-device submission, physical printer commands, live terminal provider feeds, chargebacks, lots/serials, loyalty, and barcode depth as remaining POS scope.
- Verification for the checkpoint: `git diff --check` passes; focused POS draft helper and POS route regression passes (`npm --prefix web-modern run test -- src/routes/app/pos/-sale-draft-queue.test.ts src/routes/app/pos/-index.test.tsx`, 25 pass, 0 fail); `npm --prefix web-modern run typecheck` passes; full modern web suite passes (`npm --prefix web-modern run test`, 2724 pass, 0 fail); `npm test` passes (1663 pass, 0 fail); `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; Vitest still prints the existing `environmentMatchGlobs` deprecation and localStorage experimental warnings; `ARMOSPHERA_ONE_DB=/tmp/a1-suite-pos-local-draft-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=16, kpis=4`).

### Slice 437 - POS Browser-Local Sale Draft Auto Replay
- Browser-local POS sale drafts now carry automatic replay state (`queued`, `retrying`, `retryable-failed`, `conflict-ready`, `failed`), attempt counts, failure timestamps, and needs-review block reasons while preserving the original sale idempotency key and parsed sale payload.
- The sale draft queue helper now classifies retryable transport/backend failures separately from auth, validation, closed-session, duplicate receipt/source, and schema-mismatch failures. Retryable failures can be tried again later; conflict/business failures remain visible and are not retried forever.
- The modern POS workspace now attempts one safe automatic replay for post-failed sale drafts on page load, browser online, window focus, and visible-tab return. Successful replay posts the stored payload through the existing `/api/pos/cash-sessions/:id/sales` endpoint, removes the local draft, and shows auto-replay success evidence; operator-queued drafts remain manual.
- The POS offline readiness surface now shows local draft count, auto-ready count, needs-review count, per-draft auto-replay status, attempt count, last auto failure, and review reason while still explicitly excluding live fiscal-device submission, terminal submission, physical printer commands, and true browser-offline execution.
- The ERP comparison plan now marks safe browser automatic replay/conflict surfacing for network-failed sale drafts as shipped while keeping true browser-offline execution, live fiscal-device handling, physical printer commands, live terminal feeds, chargebacks, lots/serials, loyalty, and deeper barcode operations as remaining POS scope.
- Verification for the checkpoint: `git diff --check` passes; focused POS draft helper and POS route regression passes (`npm --prefix web-modern run test -- src/routes/app/pos/-sale-draft-queue.test.ts src/routes/app/pos/-index.test.tsx`, 33 pass, 0 fail); `npm --prefix web-modern run typecheck` passes; full modern web suite passes (`npm --prefix web-modern run test`, 2732 pass, 0 fail); `npm test` passes (1663 pass, 0 fail); `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; Vitest still prints the existing `environmentMatchGlobs` deprecation and localStorage experimental warnings; `ARMOSPHERA_ONE_DB=/tmp/a1-suite-pos-auto-replay-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=16, kpis=4`).

### Slice 438 - POS Browser-Offline Sale Capture Drafts
- Browser-local POS sale drafts now support a distinct `browser-offline` queue reason that round-trips through localStorage, falls back safely for unknown legacy reasons, and is eligible for the same safe automatic replay path as network-failed drafts.
- The modern POS workspace now tracks the browser online/offline signal, shows browser connection evidence in sale capture and offline readiness, and queues a valid sale capture directly into localStorage without calling the backend when the browser is offline.
- The offline readiness surface now counts browser-offline and network-failed drafts as auto-ready while keeping operator-queued drafts manual. Draft rows render the offline queue reason, existing customer/payment/receipt evidence, auto-replay status, retry attempts, and needs-review reasons.
- Safe auto replay now covers browser-offline drafts once the browser returns online or the workspace regains focus, while still preserving closed-session, receipt/source conflict, auth, validation, and schema mismatch outcomes as reviewable non-looping states.
- The ERP comparison plan now marks browser-offline sale capture draft evidence as shipped while keeping durable local-draft to offline-replay evidence bridging, full fiscal/printer/terminal offline execution, live fiscal-device handling, live terminal feeds, chargebacks, lots/serials, loyalty, and deeper barcode operations as remaining POS scope.
- Verification for the checkpoint: initial focused POS regression exposed one stale copy assertion, then the focused POS helper and route regression passed (`npm --prefix web-modern run test -- src/routes/app/pos/-sale-draft-queue.test.ts src/routes/app/pos/-index.test.tsx`, 37 pass, 0 fail); `git diff --check` passes; `npm --prefix web-modern run typecheck` passes; full modern web suite passes (`npm --prefix web-modern run test`, 2736 pass, 0 fail across 133 files); `npm test` passes (1663 pass, 0 fail); `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; Vitest still prints the existing `environmentMatchGlobs` deprecation and localStorage experimental warnings; `ARMOSPHERA_ONE_DB=/tmp/a1-suite-pos-browser-offline-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=16, kpis=4`).

### Slice 439 - POS Local Draft Durable Replay Evidence Bridge
- Browser-local POS sale drafts now carry optional durable replay metadata: `offlineReplayItemId`, `offlineReplaySourceKey`, and `offlineReplayStatus`, with localStorage normalization that preserves known statuses and drops unknown legacy values safely.
- Local sale draft replay now derives a deterministic durable replay source key from the original sale idempotency key and records stable replay payload evidence that excludes mutable bridge metadata while retaining the stored sale payload, queue reason, receipt/customer/payment/line evidence, and explicit local-only device state.
- Manual retry and safe automatic replay now first queue a durable `pos_offline_replay_items` sale evidence row, then post the original sale payload, then mark that durable evidence replayed after sale posting succeeds.
- If sale posting fails after durable replay evidence is queued, the browser-local draft remains in localStorage linked to the queued evidence item and visible for operator review instead of losing the replay breadcrumb.
- The POS offline readiness surface now renders durable replay status/source rows for local sale drafts and merges queued/marked durable replay evidence into the local offline replay panel after replay attempts.
- The ERP comparison plan now marks durable local-draft replay evidence bridging as shipped while keeping full browser offline fiscal execution, live fiscal-device submission, physical printer commands, live terminal provider feeds, chargebacks, lots/serials, loyalty, and deeper barcode operations as remaining POS scope.
- Verification for the checkpoint: focused POS helper and route regression passes (`npm --prefix web-modern run test -- src/routes/app/pos/-sale-draft-queue.test.ts src/routes/app/pos/-index.test.tsx`, 41 pass, 0 fail); `git diff --check` passes; `npm --prefix web-modern run typecheck` passes; full modern web suite passes (`npm --prefix web-modern run test`, 2740 pass, 0 fail across 133 files); `npm test` passes (1663 pass, 0 fail); `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; Vitest still prints the existing `environmentMatchGlobs` deprecation and localStorage experimental warnings; `ARMOSPHERA_ONE_DB=/tmp/a1-suite-pos-draft-replay-bridge-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=16, kpis=4`).

### Slice 440 - POS Local Draft Replay Rejection Closure
- Local sale draft replay errors are now stage-aware, separating durable queue evidence creation, sale posting, and replay marking so a later mark-replayed failure cannot incorrectly reject a sale that already posted.
- When sale posting fails after durable replay evidence has been queued with a non-retryable closed-session, conflict, authorization, validation, schema, or client-side failure, the UI marks the durable `pos_offline_replay_items` evidence `rejected`.
- Retryable transport/backend failures keep the durable replay evidence `queued` and leave the draft eligible for the existing safe retry path instead of finalizing the evidence too early.
- Browser-local draft rows now retain rejected durable replay status alongside the needs-review state, last error, review reason, and deterministic replay source key for operator review.
- The ERP comparison plan now marks durable local-draft replay evidence rejection closure as shipped while keeping full browser offline fiscal execution, live fiscal-device submission, physical printer commands, live terminal provider feeds, chargebacks, lots/serials, loyalty, and deeper barcode operations as remaining POS scope.
- Verification for the checkpoint: focused POS helper and route regression passes (`npm --prefix web-modern run test -- src/routes/app/pos/-sale-draft-queue.test.ts src/routes/app/pos/-index.test.tsx`, 42 pass, 0 fail); `git diff --check` passes; `npm --prefix web-modern run typecheck` passes; full modern web suite passes (`npm --prefix web-modern run test`, 2741 pass, 0 fail across 133 files); `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; Vitest still prints the existing `environmentMatchGlobs` deprecation and localStorage experimental warnings; `ARMOSPHERA_ONE_DB=/tmp/a1-suite-pos-replay-reject-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=16, kpis=4`).

### Slice 441 - POS Rejected Draft Retry Guard
- Browser-local sale drafts whose durable offline replay evidence is already `rejected` now close the old retry path instead of posting the sale and then failing when the finalized replay evidence cannot be marked replayed.
- The sale replay mutation refuses rejected durable replay drafts programmatically with a stable operator-facing message, so future callers cannot bypass the disabled UI button.
- The POS offline readiness draft row now renders the rejected durable evidence state, the deterministic replay source, the existing needs-review evidence, and a disabled `Replay closed` action with guidance to create a fresh reviewed sale draft.
- The ERP comparison plan now marks rejected-draft retry guardrails as shipped while keeping full browser offline fiscal execution, live fiscal-device submission, physical printer commands, live terminal provider feeds, chargebacks, lots/serials, loyalty, and deeper barcode operations as remaining POS scope.
- Verification for the checkpoint: focused POS helper and route regression passes (`npm --prefix web-modern run test -- src/routes/app/pos/-sale-draft-queue.test.ts src/routes/app/pos/-index.test.tsx`, 42 pass, 0 fail); `git diff --check` passes; `npm --prefix web-modern run typecheck` passes; full modern web suite passes (`npm --prefix web-modern run test`, 2741 pass, 0 fail across 133 files); `npm --prefix web-modern run build` passes with the existing Vite large-chunk and Node `module.register()` deprecation warnings; Vitest still prints the existing `environmentMatchGlobs` deprecation and localStorage experimental warnings; `ARMOSPHERA_ONE_DB=/tmp/a1-suite-pos-rejected-draft-guard-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` passes (`smoke ok: Armosphera Demo Clinic, apps=16, kpis=4`).
