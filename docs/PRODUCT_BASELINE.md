...
- The Odoo comparison plan now marks the core purchase spine and first Purchase sidebar workspace as shipped while keeping vendor master/pricelists, partial receipts, landed costs, tenders, and procurement analytics in the roadmap.
...
- Verification for the checkpoint: `node --test test/purchase.test.js` pass (4 pass, 0 fail); `node --test test/purchase.test.js test/suite-dashboard-sidebar-openability.test.mjs` pass (5 pass, 0 fail); broader adjacent run `node --test test/inventory.test.js test/api.test.js test/suite-dashboard-sidebar-openability.test.mjs test/suite-routes.test.mjs` pass after installing the missing Playwright Chromium binary and rerunning the browser-dependent sidebar test; `npm run build:ui` pass with the existing Vite large-chunk warning; `npm run smoke` pass (`smoke ok: Armosphera Demo Clinic, apps=12, kpis=4`); rendered `/app/purchase` proof selected `Yerevan Hardware Supply`, filled ՀՎՀՀ `01234568`, and showed vendor-price placeholder `60000`; `git diff --check` pass; full `npm test` pass (700 pass, 0 fail).
