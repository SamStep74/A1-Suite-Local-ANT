...
- The Finance opening-balance form and posted-balance list now render side labels and submit the configured side for pending lines.
- Verification for the checkpoint: focused opening-balance/chart verification pass (`node --check server/app.js && node --check server/ledger.js && node --check test/ledger-endpoints.test.js && node --check test/opening-balances.test.js && node --check test/opening-balances-endpoints.test.js && node --test --test-concurrency=1 test/ledger-endpoints.test.js test/opening-balances.test.js test/opening-balances-endpoints.test.js`, 15 pass, 0 fail, 0 cancelled); broader finance/localization suite pass (`node --test --test-concurrency=1 test/ledger.test.js test/ledger-endpoints.test.js test/finance-reports.test.js test/vat-return-api.test.js test/armenia-chart-of-accounts.test.js test/opening-balances.test.js test/opening-balances-endpoints.test.js`, 42 pass, 0 fail, 0 cancelled); `git diff --check` pass; full `npm test` pass (644 pass, 0 fail, 0 cancelled); `npm run build:ui` pass with the existing Vite large-chunk warning; `ARMOSPHERA_ONE_DB=/tmp/a1-suite-opening-balance-side-rules-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` pass (`smoke ok: Armosphera Demo Clinic, apps=10, kpis=4`); browser smoke on `http://127.0.0.1:4196/app/finance` verifies `111` debit and `112` credit side labels, posts `1,000,000 AMD` in `111` less `(200,000 AMD)` accumulated depreciation, shows `800,000 AMD equity`, captures `/tmp/a1-suite-opening-balance-side-rules.png`, and reports no unexpected console errors after the expected pre-login `401`.
