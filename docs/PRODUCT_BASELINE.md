...
- Verification for the checkpoint: `node --check server/app.js` pass; `node --check test/api.test.js` pass; `git diff --check` pass; focused clinic following-ongoing-renewal quote release and acceptance handoff regressions (`sales can create clinic following ongoing renewal quote release packet after workflow executes`; `accountant can hand accepted following ongoing renewal quote into HayHashvapah invoice approval`) pass (2 tests after correcting the list-count assertions to use the list response shape); full `npm test` pass (465 pass, 0 fail, 0 cancelled); `npm run build:ui` pass with the existing Vite large-chunk warning; `ARMOSPHERA_ONE_DB=/tmp/a1-suite-clinic-following-ongoing-renewal-acceptance-release-id-guard-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` pass (`smoke ok: Armosphera Demo Clinic, apps=10, kpis=4`).
