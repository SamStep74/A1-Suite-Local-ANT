...
- Verification for the checkpoint: syntax checks pass for `server/db.js`, `server/app.js`, `test/api.test.js`, and `test/catalog.test.js`; focused catalog regression pass (`node --test --test-concurrency=4 --test-timeout=60000 test/catalog.test.js`, 7 pass, 0 fail); full `test/api.test.js` pass (233 pass, 0 fail); focused purchase regression pass (`node --test --test-concurrency=4 --test-timeout=60000 test/purchase.test.js`, 8 pass, 0 fail); `git diff --check` pass; `npm run build:ui` pass with the existing Vite large-chunk warning; `ARMOSPHERA_ONE_DB=/tmp/a1-suite-catalog-quote-variant-lines-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` pass (`smoke ok: Armosphera Demo Clinic, apps=12, kpis=4`); full `node --test --test-concurrency=4 --test-timeout=120000` pass (804 pass, 0 fail, 0 cancelled).
