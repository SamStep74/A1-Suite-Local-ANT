...
- The Odoo comparison plan now marks the core backend purchase spine as shipped while keeping Purchase UI, vendor master/pricelists, partial receipts, landed costs, tenders, and procurement analytics in the roadmap.
- Verification for the checkpoint: syntax checks pass for `server/app.js`, `server/db.js`, `test/purchase.test.js`, and `test/auditor-readonly-coverage.test.js`; focused purchase/inventory/AP/auditor suite pass (`node --test test/purchase.test.js test/inventory.test.js test/payables-endpoints.test.js test/auditor-readonly-coverage.test.js`, 11 pass, 0 fail, 0 cancelled); `git diff --check` pass; full `npm test` pass (685 pass, 0 fail, 0 cancelled); `npm run build:ui` pass with the existing Vite large-chunk warning; `ARMOSPHERA_ONE_DB=/tmp/a1-suite-purchase-spine-rebased-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` pass (`smoke ok: Armosphera Demo Clinic, apps=11, kpis=4`). This is a backend-only slice, so no new browser surface was added.
