...
### Slice 302 - Clinic Following Ongoing Renewal Acceptance, Draft, And Posting ID Guards
...
- Clinic/wellness following-ongoing-renewal official-invoice posting mutations now reject malformed following-ongoing-renewal HayHashvapah draft packet path IDs before draft lookup or official invoice posting packet creation.
- Submitted following-ongoing-renewal HayHashvapah draft packet IDs must be structurally safe before they can become official invoice posting evidence, backup, suite-event, or audit inputs.
- Safe unknown following-ongoing-renewal HayHashvapah draft packet IDs still preserve the existing `404` behavior, and valid posted following-ongoing-renewal draft invoices continue creating governed official invoice posting packets.
- Rejected malformed following-ongoing-renewal quote release packet, acceptance handoff, and HayHashvapah draft packet ID requests return `400`, keep submitted path/body secrets out of error bodies, and valid following-ongoing acceptance handoff, draft invoice packet, official invoice posting packet, backup, and audit behavior remains unchanged.
- Verification for the checkpoint: `node --check server/app.js` pass; `node --check test/api.test.js` pass; `git diff --check` pass; focused clinic following-ongoing-renewal acceptance, draft, and posting regressions (`accountant can hand accepted following ongoing renewal quote into HayHashvapah invoice approval`; `accountant can record following ongoing renewal HayHashvapah draft invoice packet after owner executes finance approval`; `accountant can record following ongoing renewal official HayHashvapah invoice posting packet from following ongoing draft`) pass (3 tests); full `npm test` pass (465 pass, 0 fail, 0 cancelled); `npm run build:ui` pass with the existing Vite large-chunk warning; `ARMOSPHERA_ONE_DB=/tmp/a1-suite-clinic-following-ongoing-renewal-acceptance-and-draft-id-guards-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` pass (`smoke ok: Armosphera Demo Clinic, apps=10, kpis=4`).
