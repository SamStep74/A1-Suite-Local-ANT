...
- Opening inventory seeds 12 POS barcode scanners into the main warehouse from the catalog item master, preserving tenant-specific seed IDs for non-demo organizations.
...
- Posted stock moves emit `inventory.stock_move.posted` suite and audit events with metadata-only evidence, and tenant backups now include stock locations, quants, and moves.
- Verification for the checkpoint: `node --check server/db.js`, `node --check server/app.js`, `node --check test/inventory.test.js`, and `node --check test/auditor-readonly-coverage.test.js` pass; focused inventory/auditor suite pass (`node --test test/inventory.test.js test/auditor-readonly-coverage.test.js`, 5 pass, 0 fail, 0 cancelled); impacted API/catalog/inventory/auditor/public-quote suite pass (`node --test test/api.test.js test/catalog.test.js test/inventory.test.js test/auditor-readonly-coverage.test.js test/public-quote-ratelimit.test.js`, 252 pass, 0 fail, 0 cancelled); full `npm test` pass (582 pass, 0 fail, 0 cancelled); `npm run build:ui` pass with the existing Vite large-chunk warning; `ARMOSPHERA_ONE_DB=/tmp/a1-suite-inventory-stock-ledger-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` pass (`smoke ok: Armosphera Demo Clinic, apps=10, kpis=4`).
