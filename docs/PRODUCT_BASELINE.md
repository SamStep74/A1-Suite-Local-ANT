...
- CRM lead path IDs must be lowercase alphanumeric/hyphen strings; unsafe characters and overlong route segments are rejected with `400` before conversion work can run.
...
- Rejected malformed CRM lead conversion ID requests return `400`, keep submitted path/body secrets out of error bodies, preserve no-side-effect behavior, and leave valid conversion metadata, default-title bounds, customer profile creation, and audit behavior unchanged.
