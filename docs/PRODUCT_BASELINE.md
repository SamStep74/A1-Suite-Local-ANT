...
- Verification for the checkpoint: `node --check server/app.js && node --check test/projects.test.js && node --check test/project-billing.test.js` pass; `git diff --check` pass; focused Projects path/billing metadata regressions pass (`projects: malformed path ids`, `project-billing: rejects malformed billing-preview query`, and `project-billing: rejects malformed bill-time metadata`, 3 pass); broader Projects/billing/VAT suite pass (`node --test test/projects.test.js test/project-billing.test.js test/vat-rate-versioning.test.js`, 13 pass, 0 fail, 0 cancelled); read-only subagent review pass (code/security/docs, no findings); full `npm test` pass (506 pass, 0 fail, 0 cancelled); `npm run build:ui` pass with the existing Vite large-chunk warning; `ARMOSPHERA_ONE_DB=/tmp/a1-suite-projects-path-id-guard-smoke.sqlite ARMOSPHERA_ONE_ALLOW_EGRESS=0 npm run smoke` pass (`smoke ok: Armosphera Demo Clinic, apps=10, kpis=4`).
