...
- Verification for the checkpoint: syntax checks pass for `server/app.js`, `server/db.js`, and `test/purchase.test.js`; focused purchase regression pass (`node --test --test-concurrency=4 --test-timeout=60000 test/purchase.test.js`, 6 pass, 0 fail); broader adjacent run `node --test test/inventory.test.js test/api.test.js test/suite-dashboard-sidebar-openability.test.mjs test/suite-routes.test.mjs` passed 246 non-browser tests and hit one slow-machine Playwright URL timeout, then `node --test test/suite-dashboard-sidebar-openability.test.mjs` passed on isolated rerun; `npm run build:ui` pass with the existing Vite large-chunk warning; `npm run smoke` pass (`smoke ok: Armosphera Demo Clinic, apps=12, kpis=4`); rendered `/app/purchase` proof received 1 of 3 units for `UI-PARTIAL-682374` and showed `Partial · received 1/3`; `git diff --check` pass; full `npm test` pass (702 pass, 0 fail).
