...
- Durable `pos_sale_refunds` and `pos_sale_refund_lines` with `POST /api/pos/sales/:id/refund` record full-sale refund evidence, replay by source key, reject already-refunded/unposted sales, copy original sale-line evidence, mark the sale `refunded_full`, and include tenant-backup coverage.
- Cash full-sale refund evidence reduces open-session expected cash by the original paid-cash amount; card and bank-transfer refund evidence records the reversal without changing drawer cash. This first refund slice does not restock inventory, submit fiscal refunds, print receipts, or post ledger journals.
- The modern POS workspace exposes a post-sale refund evidence form for the last posted sale, renders refund status, amount, method, cash adjustment, and `not-posted` inventory/ledger statuses, and locks duplicate refund entry after success.
...
- Browser POS:
  ...
  - customer selection, discounts, returns/refunds. First full-sale refund evidence shipped; customer-linked refunds, voids, partial refunds, and return stock flows remain.
  - barcode input.
