...
- A1 now has a first POS cash-session, sale-posting, and fiscal-handoff spine: POS app entry, role-gated workspace, tenant-scoped cash sessions, register opening, Z-report-like closeout evidence, receipt range fields, durable sale/line rows, cash expected-cash updates, source-key replay, stock delivery moves for stock-tracked fiscal items, checksum-backed local receipt handoff packets, audit events, and backup coverage.
- A1 still does not have full retail operations: offline replay, refunds/voids, receipt printing/live fiscal-device submission, cash/card split accounting, revenue/VAT/cash/bank ledger posting, lots/serials, payment terminals, loyalty, or deep barcode operations.
...
| POS | Browser POS, offline mode, cash sessions, stock sync, receipts | First cash-session/fiscal closeout spine, cash sale posting, tracked-item stock decrement, and checksum-backed fiscal receipt handoff packets shipped; offline replay, refunds/voids, receipt printing/live fiscal-device submission, payment split accounting, lots/serials/barcode depth, and revenue/VAT/cash/bank ledger posting still missing | P1 |
...
- Tests:
  ...
  - POS sale and receipt packet replay idempotently, stock updates for tracked items, and later posts balanced ledger.
...
5. POS with Armenian fiscal evidence; first cash-session closeout, cash sale posting, stock decrement, and local receipt handoff packets shipped, next focus is refund/offline/live-fiscal-device/ledger depth.
