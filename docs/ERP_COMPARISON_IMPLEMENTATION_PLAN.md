...
- A1 now has a first POS cash-session, sale-posting, fiscal-handoff, accounting, payment-split, terminal-settlement, processor-fee, refund-evidence, and pre-receipt void-evidence spine: POS app entry, role-gated workspace, tenant-scoped cash sessions, register opening, Z-report-like closeout evidence, receipt range fields, durable sale/line/payment rows, cash expected-cash updates, source-key replay, stock delivery moves for stock-tracked fiscal items, checksum-backed local receipt handoff packets, split-aware sale revenue-VAT-settlement ledger journals, refund/void revenue-VAT-settlement reversal journals, closed-session card terminal settlement and processor-fee evidence, full-sale refund evidence with tracked-line stock return moves, first single partial refund amount evidence, pre-receipt void evidence with tracked-line return moves, audit events, and backup coverage.
- A1 still does not have full retail operations: offline replay, line-level partial stock-return workflows, receipt printing/live fiscal-device submission, lots/serials, live payment-terminal provider feeds, chargeback reconciliation, loyalty, or deep barcode operations.
...
- Durable `pos_sales`, `pos_sale_lines`, and `pos_sale_payments` with `POST /api/pos/cash-sessions/:id/sales`, optional cash/card/bank-transfer payment split evidence, same-session source-key replay, duplicate receipt protection, VAT-inclusive totals, cash expected-cash updates based only on the cash portion, and stock delivery moves for tracked fiscal items.
- Modern POS sale capture panel for one-line fiscal catalog sales with optional split-payment entry and visible payment and sale ledger journal evidence while keeping fiscal-device submission, line-level partial stock returns, and offline replay deferred.
...
- Durable `pos_sale_refunds` and `pos_sale_refund_lines` with `POST /api/pos/sales/:id/refund` record full-sale and first single partial refund amount evidence, replay by source key, reject already-refunded/unposted sales, copy original sale-line evidence, create tracked-line customer return stock moves for full refunds, mark the sale `refunded_full` or `refunded`, and include tenant-backup coverage.
- Cash full-sale refund evidence reduces open-session expected cash by the original paid-cash amount; cash partial refund evidence reduces expected cash by the partial amount; card and bank-transfer refund evidence records the reversal without changing drawer cash. Tracked sale lines now restock through linked customer-to-internal return stock moves for full refunds, partial refunds preserve proportional line amount evidence without stock-return moves, sale ledger journals split revenue and output VAT across cash/bank/card-clearing settlement accounts by payment evidence, refund ledger journals post reversal evidence, and closed card sessions can post terminal settlement evidence from card clearing to bank with optional processor-fee expense evidence. Fiscal refund submission, receipt printing, live terminal feeds, and chargeback handling remain deferred.
- The modern POS workspace exposes a post-sale refund evidence form for the last posted sale, supports optional partial refund amount evidence, renders refund status, amount, method, cash adjustment, inventory-posted status, return stock move count, and ledger status, and locks duplicate refund entry after success.
...
- Browser POS:
  ...
  - customer selection, discounts, returns/refunds. First full-sale refund evidence, tracked-line return stock moves, single partial refund amount evidence, and pre-receipt void evidence shipped; customer-linked refunds and line-level partial stock returns remain.
  - barcode input.
  - stock decrement for tracked sale lines and full-sale tracked refund restock shipped; reservations, lots/serials, and partial-return inventory flows remain.
- Fiscal layer:
  - ՀԴՄ handoff packet model. First local checksum-backed handoff packet shipped; live submission remains.
  - receipt number evidence. First receipt-range closeout and sale receipt packet evidence shipped.
...
- Finance integration:
  ...
  - returns -> credit/refund journal. First balanced full-sale refund, single partial refund, and pre-receipt void reversal journals shipped; customer-linked refunds and line-level partial stock returns remain.
- Tests:
  - offline queued sale replays idempotently.
  - session cannot close with unsafe cash discrepancy metadata.
  - POS sale, receipt packet, refund evidence, and void evidence replay idempotently, stock updates for tracked sale items, cash refunds/voids adjust the open drawer, and sale/refund/void evidence posts balanced ledger journals.
...
5. POS with Armenian fiscal evidence; first cash-session closeout, cash sale posting, payment split evidence/accounting, stock decrement, local receipt handoff packets, full-sale refund evidence, single partial refund amount evidence, pre-receipt void evidence, sale/refund/void ledger journals, closed-session card terminal settlement evidence, and processor-fee reconciliation evidence shipped, next focus is offline/live-fiscal-device handling, richer returns, line-level partial stock returns, live terminal feeds, and chargeback reconciliation.
