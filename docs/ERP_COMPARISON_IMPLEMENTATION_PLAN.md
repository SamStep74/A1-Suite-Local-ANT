...
| POS | Browser POS, offline mode, cash sessions, stock sync, receipts | First cash-session/fiscal closeout spine, cash sale posting, tracked-item stock decrement, checksum-backed fiscal receipt handoff packets, full-sale refund evidence with tracked-line stock return moves, first single partial refund amount evidence, sale/refund revenue-VAT-settlement ledger journals, closed-session card terminal settlement evidence, and processor-fee reconciliation evidence shipped; offline replay, voids, line-level partial stock returns, receipt printing/live fiscal-device submission, payment split accounting, lots/serials/barcode depth, live terminal feeds, and chargeback reconciliation still missing | P1 |
...
- Modern POS sale capture panel for one-line fiscal catalog sales with visible sale ledger journal evidence while keeping fiscal-device submission, voids, line-level partial stock returns, and offline replay deferred.
...
- Finance integration:
  - POS sale -> revenue/VAT/cash/bank/card-clearing journal. First balanced sale ledger posting, card terminal clearing-to-bank settlement, and terminal processor-fee posting shipped; payment split, live terminal feeds, and chargebacks remain.
...
5. POS with Armenian fiscal evidence; first cash-session closeout, cash sale posting, stock decrement, local receipt handoff packets, full-sale refund evidence, single partial refund amount evidence, sale/refund ledger journals, closed-session card terminal settlement evidence, and processor-fee reconciliation evidence shipped, next focus is offline/live-fiscal-device handling, richer returns, line-level partial stock returns, live terminal feeds, and chargeback reconciliation.
