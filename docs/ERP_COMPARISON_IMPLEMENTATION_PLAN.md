...
- The modern POS workspace now has a browser-local sale draft queue for the first offline execution step: sale form payloads can be queued explicitly by the cashier or automatically after network/backend-unavailable failures, persisted in localStorage with the original idempotency key and customer/payment/receipt evidence, and manually retried without claiming automatic replay or fiscal-device submission.
...
- Browser POS:
  ...
  - local offline replay readiness queue evidence and first browser-local sale draft storage/manual retry shipped; automatic replay and conflict resolution remain.
...
5. POS with Armenian fiscal evidence; first cash-session closeout, cash sale posting, customer-linked sale/refund evidence and first selector, payment split evidence/accounting, stock decrement, local receipt handoff packets, local receipt print-preview evidence, offline replay queue readiness, browser-local sale draft queue/manual retry, full-sale refund evidence, single partial refund amount evidence, line-level partial stock-return refund evidence, pre-receipt void evidence, sale/refund/void ledger journals, closed-session card terminal settlement evidence, and processor-fee reconciliation evidence shipped, next focus is automatic replay/conflict resolution, live fiscal-device handling, richer returns, live terminal feeds, and chargeback reconciliation.
