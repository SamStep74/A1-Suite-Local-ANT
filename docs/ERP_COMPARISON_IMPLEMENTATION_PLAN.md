...
- A1 still does not have full retail operations: offline replay, receipt printing/live fiscal-device submission, lots/serials, live payment-terminal provider feeds, chargeback reconciliation, loyalty, or deep barcode operations.
...
| POS | Browser POS, offline mode, cash sessions, stock sync, receipts | First cash-session/fiscal closeout spine, cash sale posting, tracked-item stock decrement, checksum-backed fiscal receipt handoff packets, full-sale and line-level partial refund evidence with tracked-line stock return moves, first single partial refund amount evidence, pre-receipt void evidence with stock/cash/ledger reversal evidence, split payment evidence, split-aware sale revenue/VAT settlement journals, closed-session card terminal settlement evidence, and processor-fee reconciliation evidence shipped; offline replay, receipt printing/live fiscal-device submission, lots/serials/barcode depth, live terminal feeds, and chargeback reconciliation still missing | P1 |
...
- Durable `pos_sale_refunds` and `pos_sale_refund_lines` with `POST /api/pos/sales/:id/refund` record full-sale, first single partial refund amount, and line-level partial stock-return evidence, replay by source key, reject already-refunded/unposted sales, copy original sale-line evidence, create tracked-line customer return stock moves for full and requested partial-line refunds, mark the sale `refunded_full` or `refunded`, and include tenant-backup coverage.
...
- Browser POS:
  ...
  - customer selection, discounts, returns/refunds. First full-sale refund evidence, tracked-line return stock moves, single partial refund amount evidence, line-level partial stock returns, and pre-receipt void evidence shipped; customer-linked refunds remain.
  - barcode input.
...
- Finance integration:
  ...
  - returns -> credit/refund journal. First balanced full-sale refund, single partial refund, line-level partial stock-return refund, and pre-receipt void reversal journals shipped; customer-linked refunds remain.
- Tests:
...
5. POS with Armenian fiscal evidence; first cash-session closeout, cash sale posting, payment split evidence/accounting, stock decrement, local receipt handoff packets, full-sale refund evidence, single partial refund amount evidence, line-level partial stock-return refund evidence, pre-receipt void evidence, sale/refund/void ledger journals, closed-session card terminal settlement evidence, and processor-fee reconciliation evidence shipped, next focus is offline/live-fiscal-device handling, richer returns, live terminal feeds, and chargeback reconciliation.
