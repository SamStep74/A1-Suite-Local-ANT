...
- A1 has projects, project template evidence, recurring task evidence, tasks, subtask hierarchy evidence, task dependency evidence, milestones, time entries, project billing, project-level profitability evidence, task/product cost-basis profitability evidence, service cases, SLA policy evidence and editing UX, field service visit planning evidence, worksheet summaries, assigned-technician visit workflow, offline-safe technician dispatch queue and idempotent replay evidence, mobile/PWA technician dispatch shell evidence with map/navigation links, technician GPS/geolocation capture evidence, browser dispatch alert feed and acknowledgement evidence, schedule/location route-plan evidence, support replies, escalations, and Customer 360.
- Missing the reference ERP-style depth: native offline map tiles, native/service-worker push dispatch alerts, live distance-matrix/traffic-aware route optimization, and deeper accounting-grade cost allocation across inventory, procurement, and field-service operations.
