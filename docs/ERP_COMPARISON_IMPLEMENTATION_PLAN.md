...
- The modern POS workspace now has a browser-local sale draft queue for the first offline execution step: sale form payloads can be queued explicitly by the cashier or automatically after network/backend-unavailable failures, persisted in localStorage with the original idempotency key and customer/payment/receipt evidence, manually retried, or safely auto-replayed on load/online/focus when they were queued after failed posting. Closed-session, receipt/source conflict, auth, validation, and schema failures stay visible as needs-review evidence instead of retrying forever.
...
- Browser POS:
  ...
  - local offline replay readiness queue evidence, first browser-local sale draft storage/manual retry, and safe automatic replay/conflict surfacing for network-failed sale drafts shipped; true browser-offline execution remains.
