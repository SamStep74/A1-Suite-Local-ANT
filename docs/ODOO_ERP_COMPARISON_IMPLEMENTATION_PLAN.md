...
- This remains one of A1's largest ERP gaps, but the first catalog/inventory/purchase spine is now shipped: product master rows, stock locations, stock balances, governed stock moves, a Suite sidebar Inventory workspace, and backend RFQ/PO -> receipt -> AP bill flow. A1 still lacks warehouse operations depth, replenishment, valuation accounting, lots/serials, Purchase UI, vendor master/pricelists, tenders/blanket orders, partial receipts, landed costs, and procurement analytics.
...
| Purchase/procurement | RFQ, PO, vendor pricelists, tender/blanket orders, vendor bills | Shipped backend RFQ/PO -> receipt -> AP bill spine; Purchase UI, vendor master/pricelists, tenders, partial receipts, and landed costs still missing | P0 |
...
- Purchase:
  ...
  - Shipped core backend on 2026-06-06: RFQ/PO records, confirmation, full receipt to `WH/STOCK`, AP bill generation, idempotency, role gates, period-lock reuse, and backup inclusion.
...
- UI:
  ...
  - Purchase app in sidebar (pending; backend spine shipped first).
...
2. Inventory core.
3. Purchase/procurement backend spine (shipped on 2026-06-06); next: Purchase UI and vendor/procurement depth.
...
- Backend shipped on 2026-06-06:
  ...
  - Auditor read-only coverage, backup inclusion, period-lock blocking, sanitized malformed metadata/path guards, duplicate PO-number `409`, and idempotent retries are covered by `test/purchase.test.js`.
- Remaining:
  - Vendor master and vendor pricelists.
  - Purchase sidebar/workspace UI.
